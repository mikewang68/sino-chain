...
pub fn wens_of_sor(matches: &ArgMatches<'_>, name: &str) -> Option<u64> {
    value_of(matches, name).map(sor_to_wens)
}
...
mod tests {
    ...
    fn test_wens_of_sor() {
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(wens_of_sor(&matches, "single"), Some(50_000_000_000));
        assert_eq!(wens_of_sor(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(wens_of_sor(&matches, "single"), Some(1_500_000_000));
        assert_eq!(wens_of_sor(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(wens_of_sor(&matches, "single"), Some(30_000_000));
    }
}
