#[deprecated(
    since = "1.8.0",
    note = "Please use `sdk::stake::instruction` or `program::stake::instruction` instead"
)]
pub use sdk::stake::instruction::*;
use {
...
mod tests {
    ...
    fn process_instruction_as_one_arg(instruction: &Instruction) -> Result<(), InstructionError> {
        let accounts = instruction.accounts.iter().map(|meta| {
            Rc::new(RefCell::new(if sysvar::clock::check_id(&meta.pubkey) {
                ...
                AccountSharedData::new(0, 0, &vote_program::id())
    ...
    fn test_stake_process_instruction_decode_bail() {
        ...
        let vote_account = AccountSharedData::new_ref(0, 0, &vote_program::id());
