...
fn redelegate(
    ...
    stake.delegation.stake = stake_wens;
...
/// captures a rewards round as wens to be awarded
///  and the total points over which those wens
///  are to be distributed
//  basically read as rewards/points, but in integers instead of as an f64
#[derive(Clone, Debug, PartialEq)]
pub struct PointValue {
    pub rewards: u64, // wens to split
...
/// returns None if there's no payout or if any deserved payout is < 1 wen
fn calculate_stake_rewards(
    ...
    // don't bother trying to split if fractional wens got truncated
    if rewards == 0 {
        if let Some(inflation_point_calc_tracer) = inflation_point_calc_tracer.as_ref() {
    ...
    if (voter_rewards == 0 || staker_rewards == 0) && is_split {
        // don't collect if we lose a whole wen somewhere
...
impl<'a> StakeAccount for KeyedAccount<'a> {
    fn initialize(
        ...
        if let StakeState::Uninitialized = self.state()? {
            ...
            if rent_exempt_reserve < self.wens()? {
    ...
    fn delegate(
        ...
        match self.state()? {
            StakeState::Initialized(meta) => {
                ...
                let stake = new_stake(
                    self.wens()?.saturating_sub(meta.rent_exempt_reserve), // can't stake the rent ;)
        ...
        if let StakeState::Uninitialized = split.state()? {
            ...
            match self.state()? {
                StakeState::Stake(meta, mut stake) => {
                    ...
                    // verify enough wens for rent and more than 0 stake in new split account
                    if wens <= split_rent_exempt_reserve.saturating_sub(split.wens()?)
                        // if not full withdrawal
                        || (wens != self.wens()?
                            // verify more than 0 stake left in previous stake
                            && checked_add(wens, meta.rent_exempt_reserve)? >= self.wens()?)
                    {
                        return Err(InstructionError::InsufficientFunds);
                    }
                    // split the stake, subtract rent_exempt_balance unless
                    // the destination account already has those wens
                    // in place.
                    // this means that the new stake account will have a stake equivalent to
                    // wens minus rent_exempt_reserve if it starts out with a zero balance
                    let (remaining_stake_delta, split_stake_amount) = if wens
                        == self.wens()?
                    {
                        // If split amount equals the full source stake, the new split stake must
                        // equal the same amount, regardless of any current wen balance in the
                        ...
                        let remaining_stake_delta =
                            wens.saturating_sub(meta.rent_exempt_reserve);
                        (remaining_stake_delta, remaining_stake_delta)
                    } else {
                        // Otherwise, the new split stake should reflect the entire split
                        // requested, less any wens needed to cover the split_rent_exempt_reserve
                        (
                            wens,
                            wens - split_rent_exempt_reserve.saturating_sub(split.wens()?),
                        )
                    };
                ...
                StakeState::Initialized(meta) => {
                    ...
                    // enough wens for rent and more than 0 stake in new split account
                    if wens <= split_rent_exempt_reserve.saturating_sub(split.wens()?)
                        // if not full withdrawal
                        || (wens != self.wens()?
                            // verify more than 0 stake left in previous stake
            ...
            self.try_account_ref_mut()?.checked_sub_wens(wens)?;
            Ok(())
        } else {
            Err(InstructionError::InvalidAccountData)
        }
    ...
    fn merge(
        ...
        let wens = source_account.wens()?;
        source_account
            .try_account_ref_mut()?
            .checked_sub_wens(wens)?;
        self.try_account_ref_mut()?.checked_add_wens(wens)?;
        Ok(())
    }
    ...
    fn withdraw(
        ...
        let wens_and_reserve = checked_add(wens, reserve)?;
        // if the stake is active, we mustn't allow the account to go away
        if is_staked // line coverage for branch coverage
            && wens_and_reserve > self.wens()?
        {
            return Err(InstructionError::InsufficientFunds);
        }

        if wens != self.wens()? // not a full withdrawal
            && wens_and_reserve > self.wens()?
        {
            assert!(!is_staked);
            return Err(InstructionError::InsufficientFunds);
...
impl MergeKind {
        ...
        match stake_keyed_account.state()? {
            StakeState::Stake(meta, stake) => {
                ...
                match (status.effective, status.activating, status.deactivating) {
                    (0, 0, 0) => Ok(Self::Inactive(meta, stake_keyed_account.wens()?)),
            ...
            StakeState::Initialized(meta) => {
                Ok(Self::Inactive(meta, stake_keyed_account.wens()?))
            }
            _ => Err(InstructionError::InvalidAccountData),
        }
    }
    ...
    fn merge(
        ...
        let merged_state = match (self, source) {
            ...
            (Self::ActivationEpoch(meta, mut stake), Self::Inactive(_, source_wens)) => {
                stake.delegation.stake = checked_add(stake.delegation.stake, source_wens)?;
...
fn merge_delegation_stake_and_credits_observed(
    ...
    {
        stake.credits_observed =
            stake_weighted_credits_observed(stake, absorbed_wens, absorbed_credits_observed)
                .ok_or(InstructionError::ArithmeticOverflow)?;
    }
    stake.delegation.stake = checked_add(stake.delegation.stake, absorbed_wens)?;
    Ok(())
}
...
fn stake_weighted_credits_observed(
    ...
    if stake.credits_observed == absorbed_credits_observed {
        ...
        let total_stake = u128::from(stake.delegation.stake.checked_add(absorbed_wens)?);
        ...
        let absorbed_weighted_credits =
            u128::from(absorbed_credits_observed).checked_mul(u128::from(absorbed_wens))?;
...
pub fn redeem_rewards(
    ...
    if let StakeState::Stake(meta, mut stake) = stake_state {
        ...
        if let Some((stakers_reward, voters_reward)) = redeem_stake_rewards(
            ...
            stake_account.checked_add_wens(stakers_reward)?;
...
fn calculate_split_rent_exempt_reserve(
    ...
    let wens_per_byte_year =
        source_rent_exempt_reserve / (source_data_len + ACCOUNT_STORAGE_OVERHEAD);
    wens_per_byte_year * (split_data_len + ACCOUNT_STORAGE_OVERHEAD)
}
...
pub fn create_lockup_stake_account(
    ...
    let mut stake_account =
        AccountSharedData::new(wens, std::mem::size_of::<StakeState>(), &id());

    let rent_exempt_reserve = rent.minimum_balance(stake_account.data().len());
    assert!(
        wens >= rent_exempt_reserve,
        "wens: {} is less than rent_exempt_reserve {}",
        wens,
        rent_exempt_reserve
    );
...
fn do_create_account(
    ...
    stake_account
        .set_state(&StakeState::Stake(
            ...
            new_stake(
                wens - rent_exempt_reserve, // underflow is an error, is basically: assert!(wens > rent_exempt_reserve);
                voter_pubkey,
                &vote_state,
                activation_epoch,
                &Config::default(),
...
mod tests {
    ...
    fn test_authorized_authorize_with_custodian() {
        let staker = sdk::pubkey::new_rand();
        let custodian = sdk::pubkey::new_rand();
        let invalid_custodian = sdk::pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_stake_delegate() {
        ...
        let vote_pubkey_2 = sdk::pubkey::new_rand();
        ...
        let stake_pubkey = sdk::pubkey::new_rand();
        let stake_wens = 42;
        ...
        assert_eq!(
            ...
            Err(sdk::instruction::InstructionError::IncorrectProgramId)
        );
    ...
    fn test_stake_initialize() {
        ...
        let stake_wens = 42;
        let stake_account =
            AccountSharedData::new_ref(stake_wens, std::mem::size_of::<StakeState>(), &id());
    ...
    fn test_optional_lockup_for_stake_program_v3_and_earlier() {
        ...
        let new_custodian = sdk::pubkey::new_rand();
        assert_eq!(
            stake_keyed_account.set_lockup(
                &LockupArgs {
                    unix_timestamp: None,
    ...
    fn test_optional_lockup_for_stake_program_v4() {
        ...
        let new_withdraw_authority = sdk::pubkey::new_rand();
        assert_eq!(
    ...
    fn test_withdraw_stake() {
        ...
        // reset balance
        stake_account.borrow_mut().set_wens(stake_wens);
        ...
        // Stake some wens (available wens for withdrawals will reduce to zero)
        let vote_pubkey = sdk::pubkey::new_rand();
        let vote_account = RefCell::new(vote_state::create_account(
            &vote_pubkey,
            &sdk::pubkey::new_rand(),
        ...
        // simulate rewards
        stake_account.borrow_mut().checked_add_wens(10).unwrap();
    ...
    fn test_withdraw_stake_before_warmup() {
        ...
        // Stake some wens (available wens for withdrawals will reduce)
        let vote_pubkey = sdk::pubkey::new_rand();
        let vote_account = RefCell::new(vote_state::create_account(
            &vote_pubkey,
            &sdk::pubkey::new_rand(),
    ...
    fn test_stake_state_calculate_points_with_typical_values() {
        ...
        //  10_000_000 SOR is a big but not unreasaonable stake
        let stake = new_stake(
            native_token::sor_to_wens(10_000_000f64),
            &Pubkey::default(),
            &vote_state,
            std::u64::MAX,
            &Config::default(),
    ...
    fn test_authorize_uninit() {
        let new_authority = sdk::pubkey::new_rand();
        let stake_wens = 42;
        let stake_account = AccountSharedData::new_ref_data_with_space(
            stake_wens,
            &StakeState::default(),
    ...
    fn test_authorize_lockup() {
        let stake_authority = sdk::pubkey::new_rand();
        let stake_wens = 42;
        let stake_account = AccountSharedData::new_ref_data_with_space(
            stake_wens,
        ...
        let stake_pubkey0 = sdk::pubkey::new_rand();
        ...
        let stake_pubkey1 = sdk::pubkey::new_rand();
        assert_eq!(
            stake_keyed_account.authorize(
                &signers,
                &stake_pubkey1,
        ...
        let stake_pubkey2 = sdk::pubkey::new_rand();
        assert_eq!(
            stake_keyed_account.authorize(
                &signers0,
                &stake_pubkey2,
    ...
    fn test_authorize_with_seed() {
        let base_pubkey = sdk::pubkey::new_rand();
        let seed = "42";
    ...
    fn test_authorize_override() {
        let withdrawer_pubkey = sdk::pubkey::new_rand();
        let stake_wens = 42;
        let stake_account = AccountSharedData::new_ref_data_with_space(
            stake_wens,
            &StakeState::Initialized(Meta::auto(&withdrawer_pubkey)),
        ...
        let mallory_pubkey = sdk::pubkey::new_rand();
        let signers = vec![new_authority].into_iter().collect();
        assert_eq!(
            stake_keyed_account.authorize(
                &signers,
        ...
        let new_stake_pubkey = sdk::pubkey::new_rand();
        assert_eq!(
            stake_keyed_account.authorize(
                &signers,
                &new_stake_pubkey,
    ...
    fn test_split_source_uninitialized() {
        ...
        let split_stake_pubkey = sdk::pubkey::new_rand();
        ...
        let signers = vec![stake_pubkey].into_iter().collect();
        assert_eq!(
            stake_keyed_account.split(stake_wens / 2, &split_stake_keyed_account, &signers),
            Ok(())
        );
        assert_eq!(
    ...
    fn test_split_split_not_uninitialized() {
        ...
        let stake_account = AccountSharedData::new_ref_data_with_space(
            stake_wens,
            &StakeState::Stake(Meta::auto(&stake_pubkey), just_stake(stake_wens)),
            std::mem::size_of::<StakeState>(),
            &id(),
        )
        .expect("stake_account");
    ...
    fn test_split_with_rent() {
        ...
        for state in &[
            StakeState::Initialized(meta),
            StakeState::Stake(meta, just_stake(stake_wens - rent_exempt_reserve)),
        ] {
            let stake_account = AccountSharedData::new_ref_data_with_space(
                stake_wens,
                state,
            ...
            assert_eq!(
                stake_keyed_account.split(
                    stake_wens - rent_exempt_reserve,
                    &split_stake_keyed_account,
                    &signers
                ),
                Err(InstructionError::InsufficientFunds)
            ...
            // split account already has way enough wens
            split_stake_keyed_account
                .account
                .borrow_mut()
                .set_wens(10_000_000);
            assert_eq!(
                stake_keyed_account.split(
                    stake_wens - (rent_exempt_reserve + 1), // leave rent_exempt_reserve + 1 in original account
                    &split_stake_keyed_account,
                    &signers
                ),
                Ok(())
            ...
            if let StakeState::Stake(meta, stake) = state {
                assert_eq!(
                    ...
                    Ok(StakeState::Stake(
                        *meta,
                        Stake {
                            delegation: Delegation {
                                stake: stake_wens - rent_exempt_reserve - 1,
                                ..stake.delegation
                            },
                            ..*stake
                        }
                ...
                assert_eq!(
                    split_stake_keyed_account.account.borrow().wens(),
                    10_000_000 + stake_wens - rent_exempt_reserve - 1
                );
            }
        }
    }
    ...
    fn test_split() {
        ...
        for state in &[
            ...
            assert_eq!(
                stake_keyed_account.split(stake_wens + 1, &split_stake_keyed_account, &signers),
                Err(InstructionError::InsufficientFunds)
            );
    ...
    fn test_split_to_account_with_rent_exempt_reserve() {
        ...
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(std::mem::size_of::<StakeState>());
        let stake_wens = rent_exempt_reserve * 3; // Enough to allow half to be split and remain rent-exempt
        ...
        let state = StakeState::Stake(meta, just_stake(stake_wens - rent_exempt_reserve));
        ...
        let split_wen_balances = vec![0, 1, rent_exempt_reserve, rent_exempt_reserve + 1];
        for initial_balance in split_wen_balances {
            let split_stake_account = AccountSharedData::new_ref_data_with_space(
                initial_balance,
                &StakeState::Uninitialized,
                std::mem::size_of::<StakeState>(),
            ...
            assert_eq!(
                stake_keyed_account.account.borrow().wens()
                    + split_stake_keyed_account.account.borrow().wens(),
                stake_wens + initial_balance
            );

            if let StakeState::Stake(meta, stake) = state {
                let expected_stake =
                    stake_wens / 2 - (rent_exempt_reserve.saturating_sub(initial_balance));
                assert_eq!(
                    Ok(StakeState::Stake(
                        meta,
                        Stake {
                ...
                assert_eq!(
                    Ok(StakeState::Stake(
                        ...
                        Stake {
                            delegation: Delegation {
                                stake: stake_wens / 2 - rent_exempt_reserve,
                                ..stake.delegation
                            },
                            ..stake
                        }
    ...
    fn test_split_to_smaller_account_with_rent_exempt_reserve() {
        ...
        for initial_balance in split_wen_balances {
            ...
            if let StakeState::Stake(meta, stake) = state {
                ...
                let expected_stake = stake_wens / 2
    ...
    fn test_split_to_larger_account() {
        ...
        let stake_wens = expected_rent_exempt_reserve + 1;
        let split_amount = stake_wens - (rent_exempt_reserve + 1); // Enough so that split stake is > 0
        ...
        for initial_balance in split_wen_balances {
            ...
            let split_result =
                stake_keyed_account.split(stake_wens, &split_stake_keyed_account, &signers);
            assert_eq!(split_result, Err(InstructionError::InvalidAccountData));
        }
    }
    ...
    fn test_split_100_percent_of_source() {
        ...
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(std::mem::size_of::<StakeState>());
        let stake_wens = rent_exempt_reserve * 3; // Arbitrary amount over rent_exempt_reserve
                    ...
                    assert_eq!(
                        Ok(StakeState::Stake(
                            *meta,
                            Stake {
                                delegation: Delegation {
                                    stake: stake_wens - rent_exempt_reserve,
                                    ..stake.delegation
                                },
                                ..*stake
                            }
    ...
    #[test]
    fn test_split_100_percent_of_source_to_account_with_wens() {
        let stake_pubkey = sdk::pubkey::new_rand();
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(std::mem::size_of::<StakeState>());
        let stake_wens = rent_exempt_reserve * 3; // Arbitrary amount over rent_exempt_reserve
    ...
    fn test_split_rent_exemptness() {
        ...
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(std::mem::size_of::<StakeState>());
        let stake_wens = rent_exempt_reserve + 1;
        ...
        for state in &[
            ...
            match state {
                ...
                StakeState::Stake(_meta, stake) => {
                    // Expected stake should reflect original stake amount so that extra wens
                    // from the rent_exempt_reserve inequality do not magically activate
                    let expected_stake = stake_wens - rent_exempt_reserve;
    ...
    fn test_merge() {
        ...
        let stake_pubkey = sdk::pubkey::new_rand();
        let source_stake_pubkey = sdk::pubkey::new_rand();
        let authorized_pubkey = sdk::pubkey::new_rand();
        let stake_wens = 42;
        ...
        for state in &[
            ...
            StakeState::Stake(Meta::auto(&authorized_pubkey), just_stake(stake_wens)),
        ] {
            for source_state in &[
                StakeState::Initialized(Meta::auto(&authorized_pubkey)),
                StakeState::Stake(Meta::auto(&authorized_pubkey), just_stake(stake_wens)),
    ...
    fn test_merge_self_fails() {
        ...
        let stake_wens = rent_exempt_reserve + stake_amount;
    ...
    fn test_merge_incorrect_authorized_staker() {
        ...
        let source_stake_pubkey = sdk::pubkey::new_rand();
        let authorized_pubkey = sdk::pubkey::new_rand();
        let wrong_authorized_pubkey = sdk::pubkey::new_rand();
        let stake_wens = 42;
    ...
    fn test_merge_active_stake() {
        ...
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(std::mem::size_of::<StakeState>());
        let stake_amount = base_wens;
        let stake_wens = rent_exempt_reserve + stake_amount;
        let source_amount = base_wens;
        let source_wens = rent_exempt_reserve + source_amount;
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            "stake minimum_balance: {} wens, {} SOR",
            minimum_balance,
            minimum_balance as f64 / sdk::native_token::WENS_PER_SOR as f64
        );
    }
    ...
    fn test_authorize_delegated_stake() {
        ...
        let new_staker_pubkey = sdk::pubkey::new_rand();
        assert_eq!(
            stake_keyed_account.authorize(
                &signers,
                &new_staker_pubkey,
    ...
    fn test_redelegate_consider_balance_changes() {
        let initial_wens = 4242424242;
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(std::mem::size_of::<StakeState>());
        let withdrawer_pubkey = Pubkey::new_unique();
        let stake_wens = rent_exempt_reserve + initial_wens;
        ...
        let to_account = AccountSharedData::new_ref(1, 0, &system_program::id());
        let to_keyed_account = KeyedAccount::new(&to, false, &to_account);
        let withdraw_wens = initial_wens / 2;
        stake_keyed_account
            .withdraw(
                withdraw_wens,
                &to_keyed_account,
            ...
            .unwrap();
        let expected_balance = rent_exempt_reserve + initial_wens - withdraw_wens;
        assert_eq!(stake_keyed_account.wens().unwrap(), expected_balance);
        ...
        assert_eq!(
            stake.delegation.stake,
            stake_keyed_account.wens().unwrap() - rent_exempt_reserve,
        );
        ...
        stake_keyed_account
            .try_account_ref_mut()
            .unwrap()
            .checked_add_wens(withdraw_wens)
            .unwrap();
    ...
    #[test]
    fn test_calculate_wens_per_byte_year() {
        let rent = Rent::default();
        let data_len = 200u64;
        let rent_exempt_reserve = rent.minimum_balance(data_len as usize);
        assert_eq!(
        ...
        let even_larger_data = sdk::system_instruction::MAX_PERMITTED_DATA_LENGTH;
    ...
    fn test_merge_kind_get_if_mergeable() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            MergeKind::Inactive(meta, stake_wens)
        );

        clock.epoch = 0;
        let mut effective = 2 * initial_wens;
    ...
    fn test_merge_kind_merge() {
        ...
        let inactive = MergeKind::Inactive(Meta::default(), wens);
        ...
        assert_eq!(delegation.stake, stake.delegation.stake + wens);
