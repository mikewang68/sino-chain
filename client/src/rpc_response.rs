use {
    crate::client_error,                                            // TODO : 添加UiAccount   ConfirmedTransactionStatusWithSignature, TransactionConfirmationStatus, UiConfirmedBlock
    account_decoder::{parse_token::UiTokenAmount, UiAccount},
    sdk::{
...
pub struct RpcVersionInfo {
    /// The current version of sino-core
    pub sino_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.sino_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.sino_core.split_whitespace().next() {
