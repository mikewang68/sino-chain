...
fn full_battery_tests(
    ...
    config_payer.command = CliCommand::CreateNonceAccount {
        ...
        memo: None,
        amount: SpendAmount::Some(sor_to_wens(1000.0)),
    };
    ...
    check_balance!(sor_to_wens(1000.0), &rpc_client, &nonce_account);
    ...
    let payee_pubkey = sdk::pubkey::new_rand();
    ...
    check_balance!(sor_to_wens(900.0), &rpc_client, &nonce_account);
    check_balance!(sor_to_wens(100.0), &rpc_client, &payee_pubkey);
    ...
    check_balance!(sor_to_wens(800.0), &rpc_client, &nonce_account);
    check_balance!(sor_to_wens(200.0), &rpc_client, &payee_pubkey);
}
...
fn test_create_account_with_seed() {
    ...
    creator_config.command = CliCommand::CreateNonceAccount {
        ...
        memo: None,
        amount: SpendAmount::Some(sor_to_wens(241.0)),
    };
    process_command(&creator_config).unwrap();
    check_balance!(sor_to_wens(241.0), &rpc_client, &nonce_address);
    check_balance!(
        sor_to_wens(42.0),
        &rpc_client,
        &offline_nonce_authority_signer.pubkey(),
    ...
    check_balance!(
        sor_to_wens(4001.0 - ONE_SIG_FEE),
        &rpc_client,
        &online_nonce_creator_signer.pubkey(),
    );
    check_balance!(0, &rpc_client, &to_address);
    ...
    authority_config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(sor_to_wens(10.0)),
        to: to_address,
        from: 0,
        sign_only: true,
    ...
    check_balance!(sor_to_wens(10.0), &rpc_client, &to_address);
}
