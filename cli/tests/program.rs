...
fn test_cli_program_deploy_non_upgradeable() {
    ...
    config.command = CliCommand::Airdrop {
        pubkey: None,
        wens: 4 * minimum_balance_for_rent_exemption, // min balance for rent exemption for three programs + leftover for tx processing
    };
    ...
    assert_eq!(account0.wens, minimum_balance_for_rent_exemption);
    ...
    let account1 = rpc_client
        ...
        .unwrap();
    assert_eq!(account1.wens, minimum_balance_for_rent_exemption);
    ...
    config.command = CliCommand::Airdrop {
        pubkey: None,
        wens: 2 * minimum_balance_for_rent_exemption, // Anything over minimum_balance_for_rent_exemption should trigger err
    };
    process_command(&config).unwrap();
    config.signers = vec![&keypair, &custom_address_keypair];
    config.command = CliCommand::Deploy {
    ...
    let account2 = rpc_client
        .get_account(&custom_address_keypair.pubkey())
        .unwrap();
    assert_eq!(account2.wens, 2 * minimum_balance_for_rent_exemption);
...
fn test_cli_program_deploy_no_authority() {
    ...
    config.command = CliCommand::Airdrop {
        pubkey: None,
        wens: 100 * minimum_balance_for_programdata + minimum_balance_for_program,
    };
    config.signers = vec![&keypair];
    process_command(&config).unwrap();
...
fn test_cli_program_deploy_with_authority() {
    ...
    assert_eq!(program_account.wens, minimum_balance_for_program);
...
fn test_cli_program_close_program() {
    ...
    let programdata_wens = close_account.wens;
    ...
    assert_eq!(programdata_wens, recipient_account.wens);
}
...
fn test_cli_program_write_buffer() {
    ...
    config.command = CliCommand::Airdrop {
        pubkey: None,
        wens: 100 * minimum_balance_for_buffer,
    };
    process_command(&config).unwrap();
    ...
    assert_eq!(buffer_account.wens, minimum_balance_for_buffer_default);
    ...
    assert_eq!(buffer_account.wens, minimum_balance_for_buffer);
    assert_eq!(buffer_account.owner, bpf_loader_upgradeable::id());
    if let UpgradeableLoaderState::Buffer { authority_address } = buffer_account.state().unwrap() {
        assert_eq!(authority_address, Some(keypair.pubkey()));
    } else {
    ...
    assert_eq!(minimum_balance_for_buffer, close_account.wens);
    let recipient_pubkey = Pubkey::new_unique();
    ...
    let recipient_account = rpc_client.get_account(&recipient_pubkey).unwrap();
    assert_eq!(minimum_balance_for_buffer, recipient_account.wens);
    ...
    let pre_wens = rpc_client.get_account(&keypair.pubkey()).unwrap().wens;
    config.signers = vec![&keypair];
    config.command = CliCommand::Program(ProgramCliCommand::Close {
    ...
    assert_eq!(
        pre_wens + minimum_balance_for_buffer,
        recipient_account.wens
    );
