            ...
            SubCommand::with_name("stakes")
                ...
                .arg(
                    Arg::with_name("wens")
                        .long("wens")
                        .takes_value(false)
                        .help("Display balance in wens instead of VLX"),
                ),
        )
        .subcommand(
            ...
            SubCommand::with_name("rent")
                ...
                .arg(
                    Arg::with_name("wens")
                        .long("wens")
                        .takes_value(false)
                        .help("Display rent in wens instead of VLX"),
                ),
        )
    }
}
...
pub fn process_catchup(
    ...
    loop {
        ...
        // especially if pointing to api.meinnet-beta.sino.com at times
    ...
    let fees = if let Some(recent_blockhash) = blockhash {
        ...
        if let Some(fee_calculator) = result.value {
            CliFees::some(
                result.context.slot,
                *recent_blockhash,
                fee_calculator.wens_per_signature,
                None,
                None,
            )
        } else {
        ...
        CliFees::some(
            ...
            result.value.fee_calculator.wens_per_signature,
            None,
...
pub fn process_total_supply(rpc_client: &RpcClient, _config: &CliConfig) -> ProcessResult {
    ...
    Ok(format!("{} VLX", wens_to_sor(supply.total)))
}
...
pub fn process_ping(
    ...
    'mainloop: for seq in 0..count.unwrap_or(std::u64::MAX) {
        ...
        let build_message = |wens| {
            let ix = system_instruction::transfer(&config.signers[0].pubkey(), &to, wens);
            Message::new(&[ix], Some(&config.signers[0].pubkey()))
        };
        let (message, _) = resolve_spend_tx_and_check_account_balance(
            rpc_client,
            false,
            SpendAmount::Some(wens),
            &blockhash,
...
pub fn process_show_stakes(
    ...
    let mut program_accounts_config = RpcProgramAccountsConfig {
        account_config: RpcAccountInfoConfig {
            encoding: Some(account_decoder::UiAccountEncoding::Base64),
...
pub fn process_show_validators(
    ...
    let majority_count = vote_accounts
        ...
        .iter()
        .filter(|v| {
            v.activated_stake >= vote_program::MIN_STAKERS_TO_BE_MAJORITY
        })
        .count() as u64;
...
struct CliRentCalculation {
    ...
    pub wens_per_epoch: u64,
    pub rent_exempt_minimum_wens: u64,
    #[serde(skip)]
    pub use_wens_unit: bool,
}

impl CliRentCalculation {
    fn build_balance_message(&self, wens: u64) -> String {
        build_balance_message(wens, self.use_wens_unit, true)
    }
}

impl fmt::Display for CliRentCalculation {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let per_byte_year = self.build_balance_message(self.wens_per_byte_year);
        let per_epoch = self.build_balance_message(self.wens_per_epoch);
        let exempt_minimum = self.build_balance_message(self.rent_exempt_minimum_wens);
...
pub fn process_calculate_rent(
    ...
    let rent_exempt_minimum_wens = rent.minimum_balance(data_length);
    ...
    let wens_per_epoch = rent.due(0, data_length, years_per_epoch).wens();
    let cli_rent_calculation = CliRentCalculation {
        wens_per_byte_year: rent.wens_per_byte_year,
        wens_per_epoch,
        rent_exempt_minimum_wens,
        use_wens_unit,
    };
...
mod tests {
    use {
        ...
        sdk::signature::{write_keypair, Keypair},
        std::str::FromStr,
