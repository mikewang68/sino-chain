use {
    ...
    bpf_loader_program::{syscalls::register_syscalls, BpfError, ThisInstructionMeter},
    ...
    rbpf::{elf::Executable, verifier, vm::Config},
            ...
            SubCommand::with_name("program")
                    ...
                    SubCommand::with_name("show")
                        ...
                        .arg(
                            Arg::with_name("wens")
                                .long("wens")
                                .takes_value(false)
                                .help("Display balance in wens instead of SOL"),
                        ),
                )
                .subcommand(
                ...
                .subcommand(
                    SubCommand::with_name("close")
                        .about("Close a program or buffer account and withdraw all wens")
                        .arg(
                            Arg::with_name("account")
                                .index(1)
                                .value_name("ACCOUNT_ADDRESS")
                        ...
                        .arg(
                            pubkey!(Arg::with_name("recipient_account")
                                ...
                                "Address of the account to deposit the closed account's wens [default: the default configured keypair]"),
                        )
                        .arg(
                            Arg::with_name("wens")
                                .long("wens")
        ...
        ("show", Some(matches)) => {
            ...
            CliCommandInfo {
                command: CliCommand::Program(ProgramCliCommand::Show {
                    ...
                    use_wens_unit: matches.is_present("wens"),
                }),
                signers: vec![],
            }
        }
...
fn process_show(
    ...
    if let Some(account_pubkey) = account_pubkey {
        ...
        let programs = get_programs(rpc_client, authority_pubkey, use_wens_unit)?;
        Ok(config.output_format.formatted_string(&programs))
    } else if buffers {
        let authority_pubkey = if all { None } else { Some(authority_pubkey) };
        let buffers = get_buffers(rpc_client, authority_pubkey, use_wens_unit)?;
...
fn complete_partial_program_init(
    ...
    if account.data.is_empty() && system_program::check_id(&account.owner) {
        ...
        if account.wens < minimum_balance {
            let balance = minimum_balance - account.wens;
        ...
        } else if account.wens > minimum_balance
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!(
        "`sino-keygen recover` and the following {}-word seed phrase:",
        words
    );
...
mod tests {
    use {
        ...
        sdk::signature::write_keypair_file,
    };
