...
use cli_output::{return_signers_with_config, ReturnSignersConfig};
use evm_loader_program::{instructions::FeePayerType, scope::evm::gweis_to_wens};
...
fn get_evm_balance(rpc_client: &RpcClient, address: evm::H160) -> anyhow::Result<()> {
    ...
    let (wens, _dust) = gweis_to_wens(balance);
    let vlx = wens_to_sor(wens);
...
fn transfer(
    ...
    let ixs =
        evm_loader_program::transfer_native_to_evm_ixs(from.pubkey(), amount, evm_address);
...
fn find_block_header(
    ...
    for slot in native_slot - range..native_slot + range {
        ...
        let hash = sdk::hash::Hash::from_str(&native_block.blockhash)?;
...
fn send_raw_tx<P: AsRef<Path>>(
    ...
    let tx: evm::Transaction = sdk::program_utils::limited_deserialize(&bytes)?;
    debug!("loaded tx: {:?}", tx);

    let ix = evm_loader_program::send_raw_tx(signer.pubkey(), tx, None, FeePayerType::Evm);
...
fn call_dummy(
    ...
    let evm_tx: evm::Transaction = sdk::program_utils::limited_deserialize(&create_tx)?;
