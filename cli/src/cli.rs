use {
    ...
    clap_utils::{self, input_parsers::*, input_validators::*, keypair::*},
    cli_output::{
    ...
    remote_wallet::remote_wallet::RemoteWalletManager,
    sdk::{
        clock::{Epoch, Slot},
    ...
    vote_program::vote_state::VoteAuthorize,
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} SOR)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} SOR)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} SOR) + fee ({1} SOR)")]
...
impl CliConfig<'_> {
    fn default_keypair_path() -> String {
        cli_config::Config::default().keypair_path
    }

    fn default_json_rpc_url() -> String {
        cli_config::Config::default().json_rpc_url
    }

    fn default_websocket_url() -> String {
        cli_config::Config::default().websocket_url
    }
    ...
    pub fn compute_websocket_url_setting(
        ...
        Self::first_nonempty_setting(vec![
            ...
            (
                ...
                cli_config::Config::compute_websocket_url(&normalize_to_url_if_moniker(
                    json_rpc_cmd_url,
                )),
            ),
            (
    ...
    let response = match matches.subcommand() {
        ...
        ("rent", Some(matches)) => {
            let data_length = value_of::<RentLengthValue>(matches, "data_length")
                .unwrap()
                .length();
            let use_wens_unit = matches.is_present("wens");
            Ok(CliCommandInfo {
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of sino-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        CliCommand::Rent {
            data_length,
            use_wens_unit,
        } => process_calculate_rent(&rpc_client, config, *data_length, *use_wens_unit),
        ...
        // Withdraw wens from a nonce account
        ...
        CliCommand::ShowStakeHistory {
            use_wens_unit,
            limit_results,
        } => process_show_stake_history(&rpc_client, config, *use_wens_unit, *limit_results),
        ...
        // Request an airdrop from Sino Faucet;
        CliCommand::Airdrop { pubkey, wens } => {
            process_airdrop(&rpc_client, config, pubkey, *wens)
        }
        // Check client balance
        CliCommand::Balance {
            pubkey,
            use_wens_unit,
        } => process_balance(&rpc_client, config, pubkey, *use_wens_unit),
        ...
        CliCommand::ShowAccount {
            ...
            output_file,
            use_wens_unit,
        } => process_show_account(&rpc_client, config, pubkey, output_file, *use_wens_unit),
        CliCommand::Transfer {
            amount,
            to,
            from,
...
pub fn request_and_confirm_airdrop(
    ...
    let signature =
        rpc_client.request_airdrop_with_blockhash(to_pubkey, wens, &recent_blockhash)?;
...
mod tests {
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let test_balance =
            test_commands
                .clone()
                .get_matches_from(vec!["test", "balance", "--wens"]);
        assert_eq!(
            parse_command(&test_balance, &default_signer, &mut None).unwrap(),
            CliCommandInfo {
                command: CliCommand::Balance {
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "400 wens");

        config.command = CliCommand::Balance {
            pubkey: None,
            use_wens_unit: false,
        };
        assert_eq!(process_command(&config).unwrap(), "0.0000004 SOR");
        ...
        let new_authorized_pubkey = sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = sdk::pubkey::new_rand();
        let source_stake_account_pubkey = sdk::pubkey::new_rand();
