use {
    ...
    transaction_status::{EncodedTransaction, UiTransactionEncoding},
...
pub fn parse_airdrop(
    ...
    let wens = wens_of_sor(matches, "amount").unwrap();
    Ok(CliCommandInfo {
        command: CliCommand::Airdrop { pubkey, wens },
        signers,
    })
}
...
pub fn process_airdrop(
    ...
    println!(
        ...
        build_balance_message(wens, false, true),
    );

    let pre_balance = rpc_client.get_balance(&pubkey)?;

    let result = request_and_confirm_airdrop(rpc_client, config, &pubkey, wens);
...
pub fn process_balance(
    ...
    Ok(build_balance_message(balance, use_wens_unit, true))
}
...
pub fn process_transfer(
    ...
    let build_message = |wens| {
        let ixs = if let Some((base_pubkey, seed, program_id, from_pubkey)) = with_seed.as_ref() {
            ...
            vec![system_instruction::transfer(&from_pubkey, to, wens)].with_memo(memo)
        };
