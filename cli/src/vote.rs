use {
    ...
    sdk::{
        ...
        native_token::wens_to_sor, pubkey::Pubkey, system_instruction::SystemError,
    ...
    fn vote_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("withdraw-from-vote-account")
                .about("Withdraw wens from a vote account into a specified account")
                .arg(
                    pubkey!(Arg::with_name("vote_account_pubkey")
                        .index(1)
                        .value_name("VOTE_ACCOUNT_ADDRESS")
...
pub fn process_create_vote_account(
    ...
    let vote_account_address = if let Some(seed) = seed {
        Pubkey::create_with_seed(&vote_account_pubkey, seed, &vote_program::id())?
    } else {
        vote_account_pubkey
    };
    ...
    if !sign_only {
            ...
            rpc_client.get_account_with_commitment(&vote_account_address, config.commitment)
        {
            if let Some(vote_account) = response.value {
                let err_msg = if vote_account.owner == vote_program::id() {
...
pub fn process_show_vote_account(
    ...
    let vote_account_data = CliVoteAccount {
        account_balance: vote_account.wens,
    ...
    if !sign_only {
        ...
        if let SpendAmount::Some(withdraw_amount) = withdraw_amount {
            ...
            if balance_remaining < minimum_balance && balance_remaining != 0 {
                return Err(CliError::BadParameter(format!(
                    "Withdraw amount too large. The vote account balance must be at least {} SOL to remain rent exempt", wens_to_sor(minimum_balance)
                ))
                .into());
            }
        }
...
mod tests {
    ...
    fn test_parse_command() {
        ...
        let authed = sdk::pubkey::new_rand();
        let (keypair_file, mut tmp_file) = make_tmp_file();
        let keypair = Keypair::new();
        write_keypair(&keypair, tmp_file.as_file_mut()).unwrap();
