...
impl CrdsGossipPush {
    // pub fn num_pending(&self, crds: &RwLock<Crds>) -> usize {     //remove
...
mod test {
    ...
    fn test_prune() {
        ...
        let self_id = sdk::pubkey::new_rand();
        let origin = sdk::pubkey::new_rand();
        ...
        let low_staked_peers = (0..10).map(|_| sdk::pubkey::new_rand());
        ...
        let high_staked_peer = sdk::pubkey::new_rand();
    ...
    fn test_process_push_old_version() {
        let crds = RwLock::<Crds>::default();
        let push = CrdsGossipPush::default();
        let mut ci = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), 0);
        ci.wallclock = 1;
    ...
    fn test_refresh_active_set() {
        ...
        for _ in 0..push.num_active {
            let value2 = CrdsValue::new_unsigned(CrdsData::ContactInfo(
                ContactInfo::new_localhost(&sdk::pubkey::new_rand(), 0),
            ));
            assert_eq!(
                crds.write()
                    .unwrap()
