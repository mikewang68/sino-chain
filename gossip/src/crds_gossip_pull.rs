...
impl sdk::sanitize::Sanitize for CrdsFilter {
    fn sanitize(&self) -> std::result::Result<(), sdk::sanitize::SanitizeError> {
...
pub(crate) mod tests {
    ...
    fn test_hash_as_u64_random() {
        ...
        for _ in 0..100 {
            let hash = sdk::hash::new_rand(&mut rng);
    ...
    fn test_pulls_only_from_allowed() {
        ...
        gossip_validators.insert(sdk::pubkey::new_rand());
        let options = node.pull_options(
            &crds,
            &me.label().pubkey(),
            0,
    ...
    fn test_crds_filter_set_add() {
        ...
        let hash_values: Vec<_> = repeat_with(|| sdk::hash::new_rand(&mut rng))
            .take(1024)
            .collect();
    ...
    fn test_new_pull_request() {
        ...
        let new = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), now);
        ping_cache
            .lock()
            .unwrap()
        ...
        let offline = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), now);
    ...
    fn test_new_mark_creation_time() {
        ...
        let old = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), 0);
        ...
        crds.insert(old.clone(), now, GossipRoute::LocalMessage)
            .unwrap();
        let new = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), 0);
    ...
    fn test_process_pull_request_response() {
        ...
        let new = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), 1);
        ping_cache.mock_pong(new.id, new.gossip, Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::ContactInfo(new));
        node_crds.insert(new, 0, GossipRoute::LocalMessage).unwrap();

        let mut dest_crds = Crds::default();
        let new_id = sdk::pubkey::new_rand();
    ...
    fn test_process_pull_response() {
        ...
        let peer_pubkey = sdk::pubkey::new_rand();
