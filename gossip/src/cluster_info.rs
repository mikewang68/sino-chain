...
const MIN_STAKE_FOR_GOSSIP: u64 = sdk::native_token::LAMPORTS_PER_VLX;
...
mod tests {
    use {
        ...
        vote_program::{vote_instruction, vote_state::Vote},
        std::{
            iter::repeat_with,
    ...
    fn test_gossip_node() {
        ...
        let (node, _, _) = ClusterInfo::spy_node(sdk::pubkey::new_rand(), 0);
    ...
    fn test_handle_pull() {
        ...
        let entrypoint_pubkey = sdk::pubkey::new_rand();
        ...
        let entrypoint_pubkey2 = sdk::pubkey::new_rand();
        assert_eq!(
            (1, 0, 0),
    ...
    fn test_cluster_spy_gossip() {
        ...
        let node = Node::new_localhost();
        let (spy, _, _) = ClusterInfo::spy_node(sdk::pubkey::new_rand(), 0);
        let cluster_info = Arc::new(ClusterInfo::new(
            node.info,
            Arc::new(Keypair::new()),
            SocketAddrSpace::Unspecified,
    ...
    fn test_cluster_info_new() {
        let d = ContactInfo::new_localhost(&sdk::pubkey::new_rand(), timestamp());
        let cluster_info = ClusterInfo::new(
            d.clone(),
            Arc::new(Keypair::new()),
            SocketAddrSpace::Unspecified,
    ...
    fn new_vote_transaction<R: Rng>(rng: &mut R, slots: Vec<Slot>) -> Transaction {
        let vote = Vote::new(slots, sdk::hash::new_rand(rng));
        let ix = vote_instruction::vote(
            &Pubkey::new_unique(), // vote_pubkey
            &Pubkey::new_unique(), // authorized_voter_pubkey
            vote,
    ...
    fn test_split_gossip_messages() {
        ...
        let splits: Vec<_> =
            ClusterInfo::split_gossip_messages(PUSH_MESSAGE_MAX_PAYLOAD_SIZE, values.clone())
                .collect();
        let self_pubkey = sdk::pubkey::new_rand();
    ...
    fn test_pull_from_entrypoint_if_not_present() {
        ...
        let other_node_pubkey = sdk::pubkey::new_rand();
