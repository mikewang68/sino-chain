use {
    ...
    core::validator::{ValidatorConfig, ValidatorStartProgress},
    download_utils::{download_snapshot_archive, DownloadProgressRecord},
    genesis_utils::download_then_check_genesis_hash,
    gossip::{
        ...
        .map(|_| {
            if let Some(url) = bootstrap_config.check_vote_account.as_ref() {
                ...
                .unwrap_or_else(|err| {
                    ...
                    // incorrect `sino-validator` command-line arguments) rather than the
                    // RPC node failing.
                    //
