...
OPTIONS:
  ...
  --node-sor SOR            - Number of SOR this node has been funded from the genesis config (default: $node_sor)
...
while [[ -n $1 ]]; do
  if [[ ${1:0:1} = - ]]; then
    ...
    if [[ $1 = --label ]]; then
      ...
      no_restart=1
      shift
    elif [[ $1 = --node-sor ]]; then
      node_sor="$2"
      shift 2
    elif [[ $1 = --no-airdrop ]]; then
      airdrops_enabled=0
      shift
    # sino-validator options
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$A_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $A_CUDA ]]; then
  program=$sino_validator_cuda
else
  program=$sino_validator
fi
...
wallet() {
  (
    set -x
    $sino_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
setup_validator_accounts() {
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      echo "Adding $node_sor to validator identity account:"
      (
        set -x
        $sino_cli \
          --keypair "$A_CONFIG_DIR/faucet.json" --url "$rpc_url" \
          transfer --allow-unfunded-recipient "$identity" "$node_sor"
      ) || return $?
    fi
...
rpc_url=$($sino_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $sino_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $sino_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $sino_keygen new --no-passphrase -so "$authorized_withdrawer"
