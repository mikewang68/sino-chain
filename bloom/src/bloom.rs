...
use {
    ...
    sdk::sanitize::{Sanitize, SanitizeError},
    std::{
        cmp, fmt,
        hash::Hasher,
        marker::PhantomData,
...
mod test {
    ...
    fn test_atomic_bloom() {
        ...
        let hash_values: Vec<_> = std::iter::repeat_with(|| sdk::hash::new_rand(&mut rng))
            .take(1200)
            .collect();
        ...
        let false_positive = std::iter::repeat_with(|| sdk::hash::new_rand(&mut rng))
            .take(10_000)
    ...
    fn test_atomic_bloom_round_trip() {
        ...
        let more_hash_values: Vec<_> =
            std::iter::repeat_with(|| sdk::hash::new_rand(&mut rng))
                .take(1000)
                .collect();
        let bloom: AtomicBloom<_> = bloom.into();
        assert_eq!(bloom.num_bits, 9731);
