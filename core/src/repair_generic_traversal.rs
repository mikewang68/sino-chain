use {
    ...
    ledger::{blockstore::Blockstore, blockstore_meta::SlotMeta},
    sdk::{clock::Slot, hash::Hash},
    ...
    fn add_tree_with_missing_shreds(
        ...
        while let Some(visit) = walk.get() {
            ...
            if parent.is_some() || !is_orphan {
                ...
                let entries = sino_entry::entry::create_ticks(
                ...
                let mut shreds = ledger::blockstore::entries_to_test_shreds(
                    entries.clone(),
                    slot,
