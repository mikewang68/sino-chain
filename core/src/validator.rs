...
pub use perf::report_target_features;
use {
    crate::{
...
pub struct ValidatorConfig {
    ...
    pub evm_state_rpc_config: Option<replica_lib::triedb::Config>,
...
pub struct Validator {
    ...
    evm_state_rpc_service: Option<replica_lib::triedb::server::RunningService>,
}
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let evm_state_rpc_service = match (config.evm_state_rpc_addr.as_ref(), config.evm_state_rpc_config.as_ref(), evm_state_archive) {
            (Some(addr), Some(config), Some(archive)) => {
                ...
                match runtime {
                    Ok(runtime) => {
                        let used_storage = replica_lib::triedb::server::UsedStorage::WritableWithGC(
                            archive
                        );

                        match replica_lib::triedb::server::RunningService::start(
                            *addr,
                            config.clone(),
                            used_storage,
                            runtime,
                            ...
                            Err(err) => {
                                error!("error starting replica_lib::triedb::server {:#?}", err);
                                None
                            },
...
fn new_banks_from_ledger(
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::LoadingLedger;
    ...
    let evm_genesis_path = ledger_path.join(sdk::genesis_config::EVM_GENESIS);
...
mod tests {
    use {
        ...
        ledger::{create_new_tmp_ledger, genesis_utils::create_genesis_config_with_leader},
        runtime::snapshot_utils::EVM_STATE_DIR,
        sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
