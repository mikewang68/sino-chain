use {
    ...
    rpc::rpc_subscriptions::RpcSubscriptions,
    runtime::{
        bank::Bank,
...
impl AggregateCommitmentService {
    ...
    pub fn aggregate_commitment(
        ...
        for (wens, account) in bank.vote_accounts().values() {
            if *wens == 0 {
                continue;
            }
            ...
            for (i, a) in ancestors.iter().enumerate() {
                if *a <= root {
                    commitment
                        ...
                        .increase_rooted_stake(wens);
                } else {
                    ancestors_index = i;
                    break;
                }
            }
            rooted_stake.push((root, wens));
        }
        ...
        for vote in &vote_state.votes {
            while ancestors[ancestors_index] <= vote.slot {
                commitment
                    ...
                    .or_insert_with(BlockCommitment::default)
                    .increase_confirmation_stake(vote.confirmation_count as usize, wens);
                ancestors_index += 1;
...
mod tests {
    use {
        super::*,
        ledger::genesis_utils::{create_genesis_config, GenesisConfigInfo},
        runtime::{
            accounts_background_service::AbsRequestSender,
            bank_forks::BankForks,
            genesis_utils::{create_genesis_config_with_vote_accounts, ValidatorVoteKeypairs},
        },
        sdk::{account::Account, pubkey::Pubkey, signature::Signer},
    ...
    fn test_aggregate_commitment_for_vote_account_1() {
        ...
        for a in ancestors {
            ...
            expected.increase_rooted_stake(wens);
            assert_eq!(*commitment.get(&a).unwrap(), expected);
        }
        assert_eq!(rooted_stake[0], (root, wens));
    }
    ...
    fn test_aggregate_commitment_for_vote_account_2() {
        ...
        for a in ancestors {
            ...
            if a <= root {
                ...
                expected.increase_confirmation_stake(1, wens);
            }
            assert_eq!(*commitment.get(&a).unwrap(), expected);
        }
        assert_eq!(rooted_stake[0], (root, wens));
    ...
    fn test_aggregate_commitment_for_vote_account_3() {
        ...
        for (i, a) in ancestors.iter().enumerate() {
            if *a <= root {
                ...
                let mut expected = BlockCommitment::default();
                expected.increase_confirmation_stake(2, wens);
                assert_eq!(*commitment.get(a).unwrap(), expected);
            } else if i <= 6 {
                let mut expected = BlockCommitment::default();
                expected.increase_confirmation_stake(1, wens);
    ...
    fn test_aggregate_commitment_validity() {
        ...
        let sk1 = sdk::pubkey::new_rand();
        let pk1 = sdk::pubkey::new_rand();
        let mut vote_account1 =
            vote_state::create_account(&pk1, &sdk::pubkey::new_rand(), 0, 100);
        let stake_account1 =
            stake_state::create_account(&sk1, &pk1, &vote_account1, &genesis_config.rent, 100);
        let sk2 = sdk::pubkey::new_rand();
        let pk2 = sdk::pubkey::new_rand();
        let mut vote_account2 =
            vote_state::create_account(&pk2, &sdk::pubkey::new_rand(), 0, 50);
        let stake_account2 =
            stake_state::create_account(&sk2, &pk2, &vote_account2, &genesis_config.rent, 50);
        let sk3 = sdk::pubkey::new_rand();
        let pk3 = sdk::pubkey::new_rand();
        let mut vote_account3 =
            vote_state::create_account(&pk3, &sdk::pubkey::new_rand(), 0, 1);
        ...
        let sk4 = sdk::pubkey::new_rand();
        let pk4 = sdk::pubkey::new_rand();
        let mut vote_account4 =
            vote_state::create_account(&pk4, &sdk::pubkey::new_rand(), 0, 1);
