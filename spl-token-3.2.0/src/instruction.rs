...
pub enum TokenInstruction {
    ...
    /// will be equal to the amount of SOR in the account. If this account is
    ...
    /// amounts of SOR and Tokens will be transferred to the destination
    /// account.
    ///
    /// Accounts expected by this instruction:
    ///
    ...
    /// Close an account by transferring all its SOR to the destination account.
    ...
    /// Given a wrapped / native token account (a token account containing SOR)
    /// updates its amount field based on the account's underlying `wens`.
    /// This is useful if a non-wrapped SOR account uses `system_instruction::transfer`
    /// to move wens to a wrapped token account, and needs to have its token
    /// `amount` field updated.
    ///
    /// Accounts expected by this instruction:
    ///
    ///   0. `[writable]`  The native token account to sync with its underlying wens.
    SyncNative,
}
