...
mod tests {
    use {
        super::*,
        sdk::{account::Account, instruction::InstructionError, system_program},
    };
    ...
    fn test_verify_account_changes_owner() {
        ...
        let mallory_program_id = sdk::pubkey::new_rand();
    ...
    fn test_verify_account_changes_data_size_changed() {
        ...
        assert_eq!(
            Change::new(&alice_program_id, &sdk::pubkey::new_rand())
                .data(vec![0], vec![0, 0])
                .verify(),
            Err(InstructionError::AccountDataSizeChanged),
