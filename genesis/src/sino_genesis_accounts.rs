use {
    ...
    sdk::{genesis_config::GenesisConfig, native_token::WENS_PER_SOR},
};
...
pub const COMMUNITY_STAKER_INFOS: &[StakerInfo] = &[
    ...
    StakerInfo {
        ...
        wens: 153_333_633 * WENS_PER_SOR + 41 * WENS_PER_SOR / 100,
...
pub fn add_genesis_accounts(genesis_config: &mut GenesisConfig, mut issued_wens: u64) {
    ...
    // "one thanks" (community pool) gets 500_000_000SOR (total) - above distributions
    create_and_add_stakes(
        genesis_config,
        &StakerInfo {
            name: "one thanks",
            staker: "7vEAL3nS9CWmy1q6njUUyHE7Cf5RmyQpND6CsoHjzPiR",
            wens: (500_000_000 * WENS_PER_SOR).saturating_sub(issued_wens),
...
mod tests {
    ...
    fn test_add_genesis_accounts() {
        ...
        let wens = genesis_config
            .accounts
            .iter()
            .map(|(_, account)| account.wens)
            .sum::<u64>();

        assert_eq!(500_000_000 * WENS_PER_SOR, wens);
    }
}
