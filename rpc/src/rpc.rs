...
use {
    ...
    faucet::faucet::request_airdrop_transaction,
    ...
    spl_token::{
        // ----------------------------------------------------------------
        sino_program::program_pack::Pack,
...
use account_program::{SinoAccountType, ACCOUNT_LEN as ACCOUNT_SIZE};
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
            ...
            .map(|address| {
                if let Some(reward) = reward_hash.get(address) {
                    return Some(RpcInflationReward {
                        ...
                        amount: reward.wens.unsigned_abs(),
    ...
    fn get_fee_calculator_for_blockhash(
        ...
        let bank = self.bank(commitment);
        let wens_per_signature = bank.get_wens_per_signature_for_blockhash(blockhash);
        new_response(
            &bank,
            wens_per_signature.map(|wens_per_signature| RpcFeeCalculator {
                fee_calculator: FeeCalculator::new(wens_per_signature),
            }),
        )
    }
    ...
    fn get_largest_accounts(
        ...
        if let Some((slot, accounts)) = self.get_cached_largest_accounts(&config.filter) {
            ...
            let accounts = bank
                ...
                .into_iter()
                .map(|(address, wens)| RpcAccountBalance {
    ...
    fn get_supply(
        ...
        Ok(new_response(
            ...
            RpcSupply {
                total: total_supply,
                circulating: total_supply - non_circulating_supply.wens,
                non_circulating: non_circulating_supply.wens,
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
    ...
    fn get_filtered_program_accounts(
            ...
            .contains(&AccountIndex::ProgramId)
        {
            ...
            Ok(bank
                ...
                // zero-wen AccountSharedData::Default() after being wiped and reinitialized in later
    ...
    fn get_filtered_spl_token_accounts_by_owner(
        ...
        // in storage as a zero-wen AccountSharedData::Default() after being wiped and reinitialized in
    ...
    fn get_filtered_spl_token_accounts_by_mint(
        ...
        // as be zero-wen AccountSharedData::Default() after being wiped and reinitialized in later
    ...
    ) -> ledger::blockstore_db::Result<Vec<evm_state::Block>> {
        if let Some(bigtable_ledger_storage) = &self.bigtable_ledger_storage {
    ...
    ) -> ledger::blockstore_db::Result<Vec<evm_state::LogWithLocation>> {
    ...
    fn get_sino_accounts_by_storage_key(
        &self,
        bank: &Arc<Bank>,
        storage_key: Pubkey,
        ...
        let is_target_sino_account = |account: &AccountSharedData| -> bool {
            *account.owner() == account_program::id()
                && account.data().len() == ACCOUNT_SIZE
                && matches!(
                    SinoAccountType::try_from(account.data()),
                    Ok(SinoAccountType::Account(_account_info))
                )
        };

        self.config
            .account_indexes
            .contains(&AccountIndex::SinoAccountStorage)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::SinoAccountStorage(storage_key),
                    is_target_sino_account,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_sino_accounts_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_sino_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == account_program::id()
                && match SinoAccountType::try_from(account.data()) {
                    Ok(SinoAccountType::Account(account)) => account.owners.contains(&owner_key),
                    _ => false,
                }
        };
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::SinoAccountOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::SinoAccountOwner(owner_key),
                    is_target_sino_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_sino_relying_party_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::SinoRelyingOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::SinoRelyingOwner(owner_key),
                    is_target_sino_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
    ...
    fn get_sino_accounts_storages_by_operational_key(
        &self,
        bank: &Arc<Bank>,
        ...
        let is_target_sino_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == account_program::id()
                && match SinoAccountType::try_from(account.data()) {
                    Ok(SinoAccountType::Storage(account_storage)) => account_storage
                        .operationals
                        .iter()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::SinoAccountOperational)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::SinoAccountOperational(operational_key),
                    is_target_sino_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `sino-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_balance(
            ...
            let balance = match balance_format {
                Some(RpcBalanceFormat::Plain) | None => UiWens::Plain(balance),
                Some(RpcBalanceFormat::String) => UiWens::String(balance.to_string()),
            };
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == sdk::slot_history::Check::Found {
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        #[rpc(meta, name = "getSinoAccountsByOperationalKey")]
        fn get_sino_accounts_by_operational_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;

        #[rpc(meta, name = "getSinoAccountsByOwnerKey")]
        fn get_sino_accounts_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ...
        #[rpc(meta, name = "getSinoRelyingPartiesByOwnerKey")]
        fn get_sino_relying_parties_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;
    ...
    impl AccountsData for AccountsDataImpl {
        ...
        fn get_sino_accounts_by_owner_key(
            ...
            debug!(
                "get_sino_accounts_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::SinoAccountOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta.get_sino_accounts_by_owner_key(&bank, owner_key).transpose().map_err(|e|RpcCustomError::ScanError {
                message: e.to_string(),
            })?;
            debug!(
                "get_sino_accounts_by_owner_key sino accounts {:?}",
                accounts
            );
        ...
        fn get_sino_relying_parties_by_owner_key(
            ...
            debug!(
                "get_sino_relying_parties_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::SinoRelyingOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta.get_sino_relying_party_by_owner_key(&bank, owner_key).transpose().map_err(|e|RpcCustomError::ScanError {
                message: e.to_string(),
            })?;
            debug!(
                "get_sino_relying_parties_by_owner_key sino accounts storages {:?}",
                accounts
            );
        ...
        fn get_sino_accounts_by_operational_key(
            ...
            debug!(
                "get_sino_accounts_by_operational_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::SinoAccountOperational);
            check_index!(&meta.config, AccountIndex::SinoAccountStorage);

            let operational_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let storages =
                meta.get_sino_accounts_storages_by_operational_key(&bank, operational_key).transpose().map_err(|e|RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_sino_accounts_by_operational_key sino accounts storages {:?}",
                storages
            );

            let accounts: std::result::Result<Vec<_>,_> = storages
                .expect("sino account operational index to be activated")
                .into_iter()
                .map(|(storage, _)| {
                    meta.get_sino_accounts_by_storage_key(&bank, storage)
                        .expect("sino account storage index to be activated")
                })
                .collect();
            ...
            debug!(
                "get_sino_accounts_by_operational_key sino accounts {:?}",
                accounts
            );
...
pub mod rpc_full {
    ...
    impl Full for FullImpl {
        ...
        fn request_airdrop(
            ...
            trace!(
                "request_airdrop id={} wens={} config: {:?}",
                pubkey_str,
                wens,
                &config
            );
            ...
            let transaction =
                request_airdrop_transaction(&faucet_addr, &pubkey, wens, blockhash).map_err(
                    |err| {
...
pub fn create_test_transactions_and_populate_blockstore(
    ...
    let success_tx = sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    assert_eq!(
        ledger::blockstore_processor::process_entries_for_tests(
            &bank,
            entries,
            true,
            Some(
...
//     const TEST_MINT_WENS: u64 = 1_000_000;
...
//         assert_eq!(response, UiWens::Plain(20));
//     }
...
//         );
//         assert!(supply.circulating >= TEST_MINT_WENS);
//         assert!(supply.total >= TEST_MINT_WENS + 20);
...
//                     "wens": bank.get_minimum_balance_for_rent_exemption(0),
//                     "wensStr": bank.get_minimum_balance_for_rent_exemption(0).to_string(),
//                     "data": "",
//                     "executable": false,
//                     "rentEpoch": 0
//                 },
...
//                 "value":[{
//                     "owner": "11111111111111111111111111111111",
//                     "wens": rent_exempt_amount,
//                     "wensStr": rent_exempt_amount.to_string(),
//                     "data": ["", "base64"],
//                     "executable": false,
//                     "rentEpoch": 0
//                 },
...
//                 {
//                     "owner": "11111111111111111111111111111111",
//                     "wens": rent_exempt_amount + 1,
//                     "wensStr": (rent_exempt_amount + 1).to_string(),
...
//                     "maxWensPerSignature": 0,
//                     "minWensPerSignature": 0,
//                     "targetWensPerSignature": 0,
...
//             voting_keypair,
//             ..
//         } = create_genesis_config(TEST_MINT_WENS);
