...
use {
    ...
    sdk::{clock::Slot, pubkey::Pubkey, signature::Signature},
...
// `RpcSorPubSubInternal` is actually used by the current PubSub API implementation.
// `RpcSorPubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcSorPubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcSorPubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcSorPubSubInternal for RpcSorPubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        serial_test::serial,
        account_decoder::{parse_account_data::parse_account_data, UiAccountEncoding},
        client::rpc_response::{
    ...
    fn test_account_subscribe() {
        ...
        let new_stake_authority = sdk::pubkey::new_rand();
