    ...
    fn reset_all_sysvar_balances(&self) {
        for sysvar_id in &[
            ...
            if let Some(mut account) = self.get_account(sysvar_id) {
                let (old_data_len, old_lamports) = (account.data().len(), account.wens());
    ...
    pub fn calculate_total_accounts_stats<'a>(
        ...
        accounts.for_each(|(pubkey, account)| {
                ...
                || rent_collector.get_rent_due(account).is_exempt()
            {
                ...
                total_accounts_stats.lamports_in_rent_paying_accounts += account.wens();
                if data_len == 0 {
...
pub(crate) mod tests {
    ...
    fn test_rent_eager_collect_rent_in_partition() {
        ...
        let zero_lamport_pubkey = sdk::pubkey::new_rand();
    ...
    fn test_adjust_sysvar_balance_for_rent() {
        ...
        let excess_lamports = smaller_sample_sysvar.wens() + 999;
        smaller_sample_sysvar.set_wens(excess_lamports);
        bank.adjust_sysvar_balance_for_rent(&mut smaller_sample_sysvar);
        assert_eq!(smaller_sample_sysvar.wens(), excess_lamports);
    }
    ...
    fn test_stake_vote_account_validity() {
        ...
        let mut vote_account = bank
            ...
            .unwrap_or_default();
        let original_lamports = vote_account.wens();
        ...
        vote_account.set_wens(original_lamports);
    ...
    fn test_transfer_sysvar() {
        ...
        #[allow(deprecated)]
        let orig_lamports = bank.get_account(&sysvar::clock::id()).unwrap().wens();
