...
impl From<&NonceFull> for DurableNonceFee {
    fn from(nonce: &NonceFull) -> Self {
        match nonce.wens_per_signature() {
            Some(wens_per_signature) => Self::Valid(wens_per_signature),
            None => Self::Invalid,
        }
    }
}
...
impl DurableNonceFee {
    pub fn wens_per_signature(&self) -> Option<u64> {
        match self {
            Self::Valid(wens_per_signature) => Some(*wens_per_signature),
            Self::Invalid => None,
        }
    }
}
...
pub trait NonceInfo {
    ...
    fn wens_per_signature(&self) -> Option<u64>;
...
impl NonceInfo for NoncePartial {
    ...
    fn wens_per_signature(&self) -> Option<u64> {
        nonce_account::wens_per_signature_of(&self.account)
    }
    fn fee_payer_account(&self) -> Option<&AccountSharedData> {
        None
    }
...
impl NonceFull {
    ...
    pub fn from_partial(
        ...
        if let Some((fee_payer_address, fee_payer_account)) = fee_payer {
            ...
            fee_payer_account.set_wens(fee_payer_account.wens().saturating_add(rent_debit));
...
pub struct RewardInfo {
    ...
    pub wens: i64,          // Reward amount
    pub post_balance: u64,      // Account balance in wens after `wens` was applied
...
impl Bank {
    ...
    pub(crate) fn new_from_fields(
        ...
        if !bank.feature_set.is_active(&disable_fee_calculator::id()) {
            bank.fee_rate_governor.wens_per_signature =
        ...
        F: Fn(&Option<AccountSharedData>) -> AccountSharedData,
    {
        ...
        if self.rent_for_sysvars() {
            // When new sysvar comes into existence (with RENT_UNADJUSTED_INITIAL_BALANCE wens),
                ...
                self.rewards
                    ...
                    .map(|(_address, reward_info)| {
                        match reward_info.reward_type {
                            RewardType::Voting | RewardType::Staking => reward_info.wens,
                            _ => 0,
                        }
                    })
                    .sum::<i64>()
            ...
            stakes
                ...
                .for_each(|(stake_pubkey, delegation)| {
                    ...
                    let stake_delegation = match self.get_account_with_fixed_root(stake_pubkey) {
                        Some(stake_account) => {
                            if stake_account.owner() != &stake_program::id() {
                                invalid_stake_keys
    ...
    ///   successfully load and parse, return the wen value of one point
                            ...
                            if stakers_reward > 0 {
                                return Some((
                                    ...
                                    RewardInfo {
                                        ...
                                        wens: stakers_reward as i64,
                                        post_balance: stake_account.wens(),
        ...
        let mut vote_rewards = vote_account_rewards
            ...
            .filter_map(
                |(vote_pubkey, (mut vote_account, commission, vote_rewards, vote_needs_store))| {
                    if let Err(err) = vote_account.checked_add_wens(vote_rewards) {
                    ...
                    if vote_rewards > 0 {
                        Some((
                            ...
                            RewardInfo {
                                ...
                                wens: vote_rewards as i64,
                                post_balance: vote_account.wens(),
        ...
        if collector_fees != 0 {
            ...
            match self.deposit(&self.collector_id, deposit) {
                ...
                Err(_) => {
                    ...
                    inc_new_counter_error!("bank-burned_fee_wens", deposit as usize);
                    burn += deposit;
                }
            }
    ...
    fn process_genesis_config(&mut self, genesis_config: &GenesisConfig) {
        ...
        for (pubkey, account) in genesis_config.accounts.iter() {
            ...
            self.capitalization.fetch_add(account.wens(), Relaxed);
        }
        ...
        self.blockhash_queue.write().unwrap().genesis_hash(
            &genesis_config.hash(),
            self.fee_rate_governor.wens_per_signature,
        );
    ...
    fn add_precompiled_account_with_owner(&self, program_id: &Pubkey, owner: Pubkey) {
        ...
        let (wens, rent_epoch) = self.inherit_specially_retained_account_fields(&None);
    ...
    pub fn last_blockhash_and_wens_per_signature(&self) -> (Hash, u64) {
        let blockhash_queue = self.blockhash_queue.read().unwrap();
        let last_hash = blockhash_queue.last_hash();
        let last_wens_per_signature = blockhash_queue
            .get_wens_per_signature(&last_hash)
            .unwrap(); // safe so long as the BlockhashQueue is consistent
        (last_hash, last_wens_per_signature)
    }
    ...
    pub fn get_wens_per_signature_for_blockhash(&self, hash: &Hash) -> Option<u64> {
        let blockhash_queue = self.blockhash_queue.read().unwrap();
        blockhash_queue.get_wens_per_signature(hash)
    }
    ...
    pub fn get_fee_for_message(&self, message: &SanitizedMessage) -> Option<u64> {
        let wens_per_signature = {
            let blockhash_queue = self.blockhash_queue.read().unwrap();
            blockhash_queue.get_wens_per_signature(message.recent_blockhash())
        }
        .or_else(|| {
            self.check_message_for_nonce(message)
                .and_then(|(address, account)| {
                    NoncePartial::new(address, account).wens_per_signature()
                })
        })?;
    ...
    pub fn get_fee_for_message_with_wens_per_signature(
        &self,
        message: &SanitizedMessage,
    ...
    pub fn register_tick(&self, hash: &Hash) {
        ...
        if self.is_block_boundary(self.tick_height.load(Relaxed) + 1) {
            ...
            w_blockhash_queue.register_hash(hash, self.fee_rate_governor.wens_per_signature);
    ...
    pub fn evm_burn_fee_activated(&self) -> bool {
        self.feature_set
            .is_active(&feature_set::sino::burn_fee::id())
    }
    ...
    fn check_message_for_nonce(
            ...
            .is_active(&feature_set::nonce_must_be_authorized::ID)
        {
            ...
            .feature_set
            .is_active(&feature_set::sino::disable_durable_nonce::id())
        {
            return None;
        }
    ...
    fn execute_loaded_transaction(
            ...
            if let Some(state) = &evm_patch {
                let evm_executor = evm_state::Executor::with_config(
                    ...
                    evm_state::executor::FeatureSet::new(
                        ...
                        self.feature_set
                            .is_active(&sdk::feature_set::sino::accept_zero_gas_price_with_native_fee::id()),
                    ),
                );
                Some(evm_executor)
            } else {
        ...
        let (blockhash, wens_per_signature) = self.last_blockhash_and_wens_per_signature();
        ...
        if let Some(evm_executor) = evm_executor {
                ...
                && evm_new_error_handling
            {
                let clear_logs = self
                    .feature_set
                    .is_active(&sdk::feature_set::sino::clear_logs_on_native_error::id());
                evm_patch
                    .as_mut()
    ...
    pub fn calculate_fee(
        ...
        if tx_wide_compute_cap {
            ...
            let current_congestion = BASE_CONGESTION.max(wens_per_signature as f64);
            let congestion_multiplier = if wens_per_signature == 0 {
                0.0 // test only
            } else {
            ...
            let signature_fee = Self::get_num_signatures_in_message(message)
                .saturating_mul(fee_structure.wens_per_signature);
            let write_lock_fee = Self::get_num_write_locks_in_message(message)
                .saturating_mul(fee_structure.wens_per_write_lock);
            ...
            wens_per_signature.saturating_mul(Self::get_num_signatures_in_message(message))
        }
    }
    ...
    fn filter_program_errors_and_collect_fee(
        ...
        let results = txs
            ...
            .map(|(tx, execution_result)| {
                ...
                let (wens_per_signature, is_nonce) = durable_nonce_fee
                    .map(|durable_nonce_fee| durable_nonce_fee.wens_per_signature())
                    .map(|maybe_wens_per_signature| (maybe_wens_per_signature, true))
                    .unwrap_or_else(|| {
                        (
                ...
                let wens_per_signature =
                    wens_per_signature.ok_or(TransactionError::BlockhashNotFound)?;
    ...
    fn distribute_rent_to_validators(
        ...
        // Leftover wens after fraction calculation, will be paid to validators starting from highest stake
        // holder
        let mut leftover_wens = rent_to_be_distributed - rent_distributed_in_initial_round;

        let mut rewards = vec![];
        validator_rent_shares
            .into_iter()
            .for_each(|(pubkey, rent_share)| {
                let rent_to_be_paid = if leftover_wens > 0 {
                    leftover_wens -= 1;
                    rent_share + 1
                } else {
                    rent_share
                ...
                if !enforce_fix || rent_to_be_paid > 0 {
                    let mut account = self
                        ...
                        .unwrap_or_default();
                    if account.checked_add_wens(rent_to_be_paid).is_err() {
                        // overflow adding wens
                        self.capitalization.fetch_sub(rent_to_be_paid, Relaxed);
                        error!(
                            "Burned {} rent wens instead of sending to {}",
                            rent_to_be_paid, pubkey
                        );
                        inc_new_counter_error!(
                            "bank-burned_rent_wens",
        ...
        if enforce_fix {
            assert_eq!(leftover_wens, 0);
        } else if leftover_wens != 0 {
            warn!(
                "There was leftover from rent distribution: {}",
                leftover_wens
            );
            self.capitalization.fetch_sub(leftover_wens, Relaxed);
        }
    }
    ...
    fn fix_spv_proofs_evm(&self) -> bool {
        self.feature_set
            .is_active(&feature_set::sino::hardfork_pack::id())
    }
    ...
    pub fn load_execute_and_commit_transactions(
        ...
        let (last_blockhash, wens_per_signature) =
            self.last_blockhash_and_wens_per_signature();
    ...
    /// `n` wens where `blockhash` is the last Entry ID observed by the client.
    ...
    pub fn transfer_evm(
        ...
        let evm_tx = evm_loader_program::evm_transfer(
            *keypair,
            *to,
            nonce,
            n.into(),
        ...
        let ix = evm_loader_program::send_raw_tx(
            fee_payer.pubkey(),
            evm_tx,
            None,
            evm_loader_program::instructions::FeePayerType::Evm,
        );
    ...
    /// Technically this issues (or even burns!) new wens,
    ...
    fn withdraw(&self, pubkey: &Pubkey, wens: u64) -> Result<()> {
        match self.get_account_with_fixed_root(pubkey) {
            Some(mut account) => {
                ...
                wens
                    ...
                    .filter(|required_balance| *required_balance <= account.wens())
    ...
    pub fn verify_bank_hash(&self, test_hash_calculation: bool) -> bool {
        self.rc.accounts.verify_bank_hash_and_wens(
            self.slot(),
            &self.ancestors,
            self.capitalization(),
    ...
    pub fn update_accounts_hash_with_index_option(
        ...
        if total_wens != self.capitalization() {
            datapoint_info!(
                "capitalization_mismatch",
                ("slot", self.slot(), i64),
                ("calculated_wens", total_wens, i64),
            ...
            panic!(
                "capitalization_mismatch. slot: {}, calculated_wens: {}, capitalization: {}",
                self.slot(),
                total_wens,
                self.capitalization()
            );
    ...
    /// A snapshot bank should be purged of 0 wen accounts which are not part of the hash
    ...
    fn apply_feature_activations(
        ...
        if !debug_do_not_add_builtins {
            ...
            self.reconfigure_token2_native_mint(
                new_feature_activations.contains(&feature_set::sino::hardfork_pack::id()),
            );
        }
    ...
    fn reset_all_sysvar_balances(&self) {
        for sysvar_id in &[
            ...
            if let Some(mut account) = self.get_account(sysvar_id) {
                let (old_data_len, old_wens) = (account.data().len(), account.wens());
    ...
    fn replace_program_account(
        ...
        if let Some(old_account) = self.get_account_with_fixed_root(old_address) {
            if let Some(new_account) = self.get_account_with_fixed_root(new_address) {
                ...
                // Burn wens in the old account
                self.capitalization
                    .fetch_sub(old_account.wens(), Relaxed);
    ...
    fn reconfigure_token2_native_mint(&mut self, reconfigure_token2_native_mint_sino: bool) {
        ...
        if reconfigure_token2_native_mint {
            ...
            {
                if existing_native_mint_account.owner() == &sdk::system_program::id() {
                    native_mint_account.set_wens(existing_native_mint_account.wens());
                    true
                } else {
                    false
                }
                ...
                self.capitalization
                    .fetch_add(native_mint_account.wens(), Relaxed);
                true
            };
        ...
        if purge_window_epoch {
            for reward_pubkey in self.rewards_pool_pubkeys.iter() {
                if let Some(mut reward_account) = self.get_account_with_fixed_root(reward_pubkey) {
                    if reward_account.wens() == u64::MAX {
                        reward_account.set_wens(0);
                        self.store_account(reward_pubkey, &reward_account);
                        // Adjust capitalization.... it has been wrapping, reducing the real capitalization by 1-wen
    ...
    pub fn calculate_total_accounts_stats<'a>(
        ...
        accounts.for_each(|(pubkey, account)| {
                ...
                || rent_collector.get_rent_due(account).is_exempt()
            {
                ...
                total_accounts_stats.wens_in_rent_paying_accounts += account.wens();
                if data_len == 0 {
...
pub struct TotalAccountsStats {
    ...
    /// Total amount of wens in rent paying accounts
    pub wens_in_rent_paying_accounts: u64,
}
...
pub(crate) mod tests {
    ...
    use {
        ...
        crate::{
            ...
            genesis_utils::{
                activate_all_features, bootstrap_validator_stake_wens,
    ...
    fn test_nonce_info() {
        ...
        let rent_collected_nonce_account = {
            ...
            account.set_wens(nonce_account.wens() - TEST_RENT_DEBIT);
            account
        };
        let rent_collected_from_account = {
            let mut account = from_account.clone();
            account.set_wens(from_account.wens() - TEST_RENT_DEBIT);
            account
        };
        ...
        rent_debits.insert(
            &from_address,
            TEST_RENT_DEBIT,
            rent_collected_from_account.wens(),
        );
        rent_debits.insert(
            &nonce_address,
            TEST_RENT_DEBIT,
            rent_collected_nonce_account.wens(),
        );
        ...
        // NonceFull create + NonceInfo impl
        {
            ...
            assert_eq!(full.wens_per_signature(), Some(wens_per_signature));
            assert_eq!(
    ...
    #[allow(clippy::float_cmp)]
    fn test_bank_new() {
        let dummy_leader_pubkey = sdk::pubkey::new_rand();
        let dummy_leader_stake_wens = bootstrap_validator_stake_wens();
        let mint_wens = 10_000;
        ...
        assert_eq!(bank.get_balance(&mint_keypair.pubkey()), mint_wens);
        assert_eq!(
            bank.get_balance(&voting_keypair.pubkey()),
            dummy_leader_stake_wens /* 1 token goes to the vote account associated with dummy_leader_wens */
        );
        ...
        assert_eq!(rent.wens_per_byte_year, 5);
    }
    ...
    fn test_credit_debit_rent_no_side_effect_on_hash() {
        ...
        system_program_account.set_wens(
    ...
    fn test_store_account_and_update_capitalization_missing() {
        ...
        let some_wens = 400;
        let account = AccountSharedData::new(some_wens, 0, &system_program::id());

        assert_capitalization_diff(
            &bank,
            || bank.store_account_and_update_capitalization(&pubkey, &account),
            |old, new| assert_eq!(old + some_wens, new),
        );
    ...
    fn test_store_account_and_update_capitalization_increased() {
        let old_wens = 400;
        let (genesis_config, mint_keypair) = create_genesis_config(old_wens);
        let bank = Bank::new_for_tests(&genesis_config);
        let pubkey = mint_keypair.pubkey();

        let new_wens = 500;
        let account = AccountSharedData::new(new_wens, 0, &system_program::id());
    ...
    fn test_store_account_and_update_capitalization_unchanged() {
        let wens = 400;
        let (genesis_config, mint_keypair) = create_genesis_config(wens);
        let bank = Bank::new_for_tests(&genesis_config);
        let pubkey = mint_keypair.pubkey();

        let account = AccountSharedData::new(wens, 1, &system_program::id());
    ...
    fn test_rent_distribution() {
        ...
        let bootstrap_validator_pubkey = sdk::pubkey::new_rand();
        let bootstrap_validator_stake_wens = 30;
        ...
        let validator_1_pubkey = sdk::pubkey::new_rand();
        let validator_1_stake_wens = 20;
        ...
        let validator_2_pubkey = sdk::pubkey::new_rand();
        let validator_2_stake_wens = 20;
        ...
        let validator_3_pubkey = sdk::pubkey::new_rand();
        let validator_3_stake_wens = 30;
        ...
        let bootstrap_validator_portion =
            ((bootstrap_validator_stake_wens * rent_to_be_distributed) as f64 / 100.0) as u64
                + 1; // Leftover wen
        assert_eq!(
        ...
        let validator_1_portion =
            ((validator_1_stake_wens * rent_to_be_distributed) as f64 / 100.0) as u64 + tweak_1;
        assert_eq!(
        ...
        let validator_2_portion =
            ((validator_2_stake_wens * rent_to_be_distributed) as f64 / 100.0) as u64 + tweak_2;
        assert_eq!(
        ...
        let validator_3_portion =
            ((validator_3_stake_wens * rent_to_be_distributed) as f64 / 100.0) as u64 + 1;
        assert_eq!(
    ...
    fn test_distribute_rent_to_validators_overflow() {
        ...
        let validator_pubkey = sdk::pubkey::new_rand();
        let mut genesis_config =
            create_genesis_config_with_leader(10, &validator_pubkey, VALIDATOR_STAKE)
                .genesis_config;

        let bank = Bank::new_for_tests(&genesis_config);
        let old_validator_wens = bank.get_balance(&validator_pubkey);
        bank.distribute_rent_to_validators(&bank.vote_accounts(), RENT_TO_BE_DISTRIBUTED);
        let new_validator_wens = bank.get_balance(&validator_pubkey);
        assert_eq!(
            new_validator_wens,
            old_validator_wens + RENT_TO_BE_DISTRIBUTED
        );
        ...
        let old_validator_wens = bank.get_balance(&validator_pubkey);
        let new_validator_wens = std::panic::catch_unwind(|| {
            bank.distribute_rent_to_validators(&bank.vote_accounts(), RENT_TO_BE_DISTRIBUTED);
            bank.get_balance(&validator_pubkey)
        });

        if let Ok(new_validator_wens) = new_validator_wens {
    ...
    fn test_rent_exempt_executable_account() {
        ...
        let account_pubkey = sdk::pubkey::new_rand();
        let account_balance = 1;
        let data_len = 12345; // use non-zero data len to also test accounts_data_len
        let mut account =
            AccountSharedData::new(account_balance, data_len, &sdk::pubkey::new_rand());
    ...
    fn test_rent_complex() {
        ...
        assert_eq!(account10.wens(), 929 - magic_rent_number);
    ...
    fn test_rent_eager_across_epoch_without_gap_under_multi_epoch_cycle() {
        let leader_pubkey = sdk::pubkey::new_rand();
        let leader_wens = 3;
        let mut genesis_config =
            create_genesis_config_with_leader(5, &leader_pubkey, leader_wens).genesis_config;
        genesis_config.cluster_type = ClusterType::MainnetBeta;
    ...
    fn map_to_test_bad_range() -> std::collections::BTreeMap<Pubkey, i8> {
        ...
        map.insert(sdk::pubkey::new_rand(), 1);
        map
    }
    ...
    fn test_rent_eager_collect_rent_in_partition() {
        ...
        let zero_wen_pubkey = sdk::pubkey::new_rand();
        let rent_due_pubkey = sdk::pubkey::new_rand();
        let rent_exempt_pubkey = sdk::pubkey::new_rand();

        let mut bank = Arc::new(Bank::new_for_tests(&genesis_config));
        let zero_wens = 0;
        let little_wens = 1234;
        let large_wens = 123_456_789;
        let rent_collected = 22;

        bank.store_account(
            &zero_wen_pubkey,
            &AccountSharedData::new(zero_wens, 0, &Pubkey::default()),
        );
        bank.store_account(
            &rent_due_pubkey,
            &AccountSharedData::new(little_wens, 0, &Pubkey::default()),
        );
        bank.store_account(
            &rent_exempt_pubkey,
            &AccountSharedData::new(large_wens, 0, &Pubkey::default()),
        );
        ...
        assert_eq!(
            bank.get_account(&rent_due_pubkey).unwrap().wens(),
            little_wens
        );
    ...
    #[test]
    fn test_rent_eager_collect_rent_zero_wen_deterministic() {
        sino_logger::setup();
        ...
        let zero_wens = 0;
        let data_len = 12345; // use non-zero data len to also test accounts_data_len
        let account = AccountSharedData::new(zero_wens, data_len, &Pubkey::default());
        bank1_with_zero.store_account(&zero_wen_pubkey, &account);
        bank1_without_zero.store_account(&zero_wen_pubkey, &account);
    ...
    fn test_bank_update_vote_stake_rewards() {
        ...
        assert!(
            ((bank1.get_balance(&stake_id) - stake_account.wens() + bank1.get_balance(&vote_id)
                - vote_account.wens()) as f64
        ...
        assert_eq!(
            *bank1.rewards.read().unwrap(),
            vec![(
                ...
                RewardInfo {
                    reward_type: RewardType::Staking,
                    wens: (rewards.validator_point_value * validator_points as f64) as i64,
    ...
    fn do_test_bank_update_rewards_determinism() -> u64 {
        ...
        let vote_id = sdk::pubkey::new_rand();
        let mut vote_account =
            vote_state::create_account(&vote_id, &sdk::pubkey::new_rand(), 50, 100);
    ...
    // Test that purging 0 wens accounts works.
    #[test]
    fn test_purge_empty_accounts() {
        ...
        assert_eq!(bank0.get_account(&keypair.pubkey()).unwrap().wens(), 10);
    ...
    fn test_one_tx_two_out_atomic_fail() {
        ...
        assert_eq!(
            ...
            TransactionError::InstructionError(1, SystemError::ResultWithNegativeWens.into())
        );
    ...
    fn test_detect_failed_duplicate_transactions() {
        ...
        // The wens didn't move, but the from address paid the transaction fee.
    ...
    fn test_transfer_to_sysvar() {
        ...
        let normal_pubkey = sdk::pubkey::new_rand();
    ...
    fn test_bank_withdraw_from_nonce_account() {
        ...
        genesis_config.rent.wens_per_byte_year = 42;
        let bank = Bank::new_for_tests(&genesis_config);
    ...
    fn test_bank_tx_fee() {
        ...
        let leader = sdk::pubkey::new_rand();
        let GenesisConfigInfo {
            mut genesis_config,
            mint_keypair,
            ..
        ...
        assert_eq!(
            *bank.rewards.read().unwrap(),
            vec![(
                ...
                RewardInfo {
                    reward_type: RewardType::Fee,
                    wens: expected_fee_collected as i64,
        ...
        // Profit! 2 transaction signatures processed at 3 wens each
        assert_eq!(
            bank.get_balance(&leader),
    ...
    fn test_bank_blockhash_fee_structure() {
        ...
        genesis_config
            .fee_rate_governor
            .target_wens_per_signature = 1000;
        genesis_config.fee_rate_governor.target_signatures_per_slot = 1;

        let mut bank = Bank::new_for_tests(&genesis_config);
        goto_end_of_slot(&mut bank);
        let cheap_blockhash = bank.last_blockhash();
        let cheap_wens_per_signature = bank.get_wens_per_signature();
        assert_eq!(cheap_wens_per_signature, 0);

        let mut bank = Bank::new_from_parent(&Arc::new(bank), &leader, 1);
        bank.deactivate_feature(&tx_wide_compute_cap::id());
        goto_end_of_slot(&mut bank);
        let expensive_blockhash = bank.last_blockhash();
        let expensive_wens_per_signature = bank.get_wens_per_signature();
        assert!(cheap_wens_per_signature < expensive_wens_per_signature);
        ...
        assert_eq!(
            bank.get_balance(&mint_keypair.pubkey()),
            initial_mint_balance - 1 - cheap_wens_per_signature
        );
        ...
        assert_eq!(
            bank.get_balance(&mint_keypair.pubkey()),
            initial_mint_balance - 1 - expensive_wens_per_signature
        );
    }
    ...
    fn test_filter_program_errors_and_collect_fee() {
        ...
        assert_eq!(
            ...
            initial_balance
                + bank
                    .fee_rate_governor
                    .burn(bank.fee_rate_governor.wens_per_signature * 2)
                    .0
        );
    ...
    fn test_readonly_accounts() {
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config_with_leader(500, &sdk::pubkey::new_rand(), 0);
        let bank = Bank::new_for_tests(&genesis_config);

        let vote_pubkey0 = sdk::pubkey::new_rand();
        let vote_pubkey1 = sdk::pubkey::new_rand();
        let vote_pubkey2 = sdk::pubkey::new_rand();
    ...
    fn test_interleaving_locks_evm_tx() {
        ...
        fn fund_evm(from_keypair: &Keypair, hash: Hash, wens: u64) -> Transaction {
            let tx = evm_loader_program::processor::dummy_call(0).0;
            let from_pubkey = from_keypair.pubkey();
            let instructions = evm_loader_program::transfer_native_to_evm_ixs(
                from_pubkey,
                wens,
                tx.caller().unwrap(),
            );
        ...
        let create_tx = |from_keypair: &Keypair, hash: Hash, nonce: usize| {
            let from_pubkey = from_keypair.pubkey();
            let instruction = evm_loader_program::send_raw_tx(
                from_pubkey,
                evm_loader_program::processor::dummy_call(nonce).0,
                None,
                evm_loader_program::instructions::FeePayerType::Evm,
            );
    ...
    fn test_evm_second_tx_with_same_nonce() {
        ...
        bank.activate_feature(&feature_set::sino::native_swap_in_evm_history::id());
        bank.activate_feature(&feature_set::sino::evm_new_error_handling::id());
        bank.activate_feature(&feature_set::sino::evm_instruction_borsh_serialization::id());
        let recent_hash = genesis_config.hash();
    ...
    fn test_evm_revert_tx() {
        ...
        fn fund_evm_with_revert(
            ...
            let from_pubkey = from_keypair.pubkey();
            let mut instructions = evm_loader_program::transfer_native_to_evm_ixs(
                from_pubkey,
                wens,
                receiver,
            );
            ...
            let ix = evm_loader_program::free_ownership(s.pubkey());
        ...
        let state_swapper = evm_state
            .get_account_state(*evm_loader_program::precompiles::ETH_TO_SOR_ADDR)
            .unwrap_or_default();
    ...
    fn test_evm_no_revert_tx_on_new_errorhandling() {
        ...
        assert_eq!(
            state.balance,
            evm_loader_program::scope::evm::wens_to_gwei(20000)
        ); // 10^9 times bigger
        let state_swapper = evm_state
            .get_account_state(*evm_loader_program::precompiles::ETH_TO_SOR_ADDR)
            .unwrap_or_default();
    ...
    fn test_bank_hash_internal_state_verify_transfer_evm() {
        ...
        bank0.activate_feature(&feature_set::sino::evm_instruction_borsh_serialization::id());
        ...
        let bank2 = Bank::new_from_parent(&bank0, &sdk::pubkey::new_rand(), 1);
        ...
        let bank3 = Bank::new_from_parent(&bank0, &sdk::pubkey::new_rand(), 2);
    ...
    fn test_bank_update_sysvar_account() {
        ...
        let dummy_clock_id = sdk::pubkey::new_rand();
    ...
    fn test_zero_signatures() {
        ...
        let (genesis_config, mint_keypair) = create_genesis_config(500);
        let mut bank = Bank::new_for_tests(&genesis_config);
        bank.fee_rate_governor.wens_per_signature = 2;
        let key = Keypair::new();
    ...
    fn test_bank_inherit_tx_count() {
        ...
        // Bank 2
        let bank2 = Bank::new_from_parent(&bank0, &sdk::pubkey::new_rand(), 2);
        ...
        let bank6 = Bank::new_from_parent(&bank1, &sdk::pubkey::new_rand(), 3);
        assert_eq!(bank1.transaction_count(), 1);
    ...
    fn test_bank_inherit_fee_rate_governor() {
        ...
        genesis_config
            .fee_rate_governor
            .target_wens_per_signature = 123;

        let bank0 = Arc::new(Bank::new_for_tests(&genesis_config));
        let bank1 = Arc::new(new_from_parent(&bank0));
        assert_eq!(
            bank0.fee_rate_governor.target_wens_per_signature / 2,
            bank1
                .fee_rate_governor
                .create_fee_calculator()
                .wens_per_signature
    ...
    fn test_bank_vote_accounts() {
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config_with_leader(500, &sdk::pubkey::new_rand(), 1);
        let bank = Arc::new(Bank::new_for_tests(&genesis_config));
    ...
    fn test_bank_fees_account() {
        ...
        assert_eq!(
            bank.fee_rate_governor.wens_per_signature,
            fees.fee_calculator.wens_per_signature
        );
        assert_eq!(fees.fee_calculator.wens_per_signature, 12345);
    }
    ...
    fn test_is_delta_with_no_committables() {
        ...
        assert_eq!(
            bank.transfer(10_001, &mint_keypair, &sdk::pubkey::new_rand()),
            Err(TransactionError::InstructionError(
                0,
                SystemError::ResultWithNegativeWens.into(),
            ))
    ...
    fn test_bank_get_program_accounts() {
        ...
        assert!(
            genesis_accounts
                .iter()
                .any(|(pubkey, _, _)| sdk::sysvar::is_sysvar_id(pubkey)),
            "no sysvars found"
        );
        ...
        // Accounts with 0 wens should be filtered out by Accounts::load_by_program()
        let pubkey2 = sdk::pubkey::new_rand();
    ...
    fn test_add_duplicate_static_program() {
        ...
        let vote_loader_account = bank.get_account(&vote_program::id()).unwrap();
        bank.add_builtin(
            "vote_program",
            &vote_program::id(),
            mock_vote_processor,
        );
        let new_vote_loader_account = bank.get_account(&vote_program::id()).unwrap();
    ...
    fn test_add_instruction_processor_for_existing_unrelated_accounts() {
        ...
        let ((vote_id, vote_account), (stake_id, stake_account)) =
            crate::stakes::tests::create_staked_node_accounts(1_0000);
        bank.capitalization
            .fetch_add(vote_account.wens() + stake_account.wens(), Relaxed);
    ...
    {
        let (mut genesis_config, mint_keypair) = create_genesis_config(supply_wens);
        genesis_config.rent.wens_per_byte_year = 0;
    ...
    fn test_nonce_transaction() {
        ...
        assert_eq!(
            ...
            Err(TransactionError::InstructionError(
                1,
                system_instruction::SystemError::ResultWithNegativeWens.into(),
            ))
        );
    ...
    fn test_nonce_authority() {
        ...
        debug!("{:?}", nonce_tx);
        let initial_custodian_balance = custodian_account.wens();
        assert_eq!(
            bank.process_transaction(&nonce_tx),
            Err(TransactionError::BlockhashNotFound),
        );
    ...
    fn test_transaction_with_duplicate_accounts_in_instruction() {
        ...
        let from_pubkey = sdk::pubkey::new_rand();
        let to_pubkey = sdk::pubkey::new_rand();
    ...
    fn test_account_ids_after_program_ids() {
        ...
        let instruction =
            Instruction::new_with_bincode(vote_program::id(), &10, account_metas);
        let mut tx = Transaction::new_signed_with_payer(
            &[instruction],
            Some(&mint_keypair.pubkey()),
            &[&mint_keypair],
        ...
        tx.message.account_keys.push(sdk::pubkey::new_rand());
        ...
        let result = bank.process_transaction(&tx);
        assert_eq!(result, Ok(()));
        let account = bank.get_account(&vote_program::id()).unwrap();
    ...
    fn test_program_id_as_payer() {
        ...
        tx.message.account_keys.push(vote_program::id());
        ...
        let keys: Vec<_> = (0..max_keys)
            ...
            .map(|_| {
                ...
                let balance = if thread_rng().gen_ratio(9, 10) {
                    let wens = if thread_rng().gen_ratio(1, 5) {
                    ...
                    let account = AccountSharedData::new(wens, space, &owner);
    ...
    fn test_same_program_id_uses_unqiue_executable_accounts() {
        fn nested_processor(
            ...
            assert_eq!(42, account.wens().unwrap());
            account.try_account_ref_mut()?.checked_add_wens(1)?;
            Ok(())
        }
        ...
        // Add a new program
        let program1_pubkey = sdk::pubkey::new_rand();
        bank.add_builtin("program", &program1_pubkey, nested_processor);

        // Add a new program owned by the first
        let program2_pubkey = sdk::pubkey::new_rand();
    ...
    fn test_clean_nonrooted() {
        ...
        bank3.deposit(&pubkey1, some_wens + 1).unwrap();
    ...
    fn test_shrink_candidate_slots_cached() {
        ...
        bank1.deposit(&pubkey1, some_wens).unwrap();
        bank1.deposit(&pubkey2, some_wens).unwrap();
        goto_end_of_slot(Arc::<Bank>::get_mut(&mut bank1).unwrap());
        bank1.freeze();
        bank1.squash();
        // Flush now so that accounts cache cleaning doesn't clean up bank 0 when later
    ...
    fn test_process_stale_slot_with_budget() {
        ...
        bank.deposit(&pubkey1, some_wens).unwrap();
        bank.deposit(&pubkey2, some_wens).unwrap();
    ...
    fn test_add_builtin_loader_no_overwrite() {
        ...
        let slot = 123;
        let loader_id = sdk::pubkey::new_rand();
    ...
    fn test_reconfigure_token2_native_mint() {
        ...
        let mut genesis_config =
            create_genesis_config_with_leader(5, &sdk::pubkey::new_rand(), 0).genesis_config;
    ...
    fn test_bank_executor_cache() {
        ...
        // Check inheritance
        let bank = Bank::new_from_parent(&Arc::new(bank), &sdk::pubkey::new_rand(), 1);
        let executors = bank.get_executors(accounts);
    ...
    fn test_program_replacement() {
        ...
        // Wens in the old token account were burnt
    ...
    fn test_adjust_sysvar_balance_for_rent() {
        ...
        assert_eq!(smaller_sample_sysvar.wens(), 1);
        bank.adjust_sysvar_balance_for_rent(&mut smaller_sample_sysvar);
        assert_eq!(
            smaller_sample_sysvar.wens(),
        ...
        assert!(smaller_sample_sysvar.wens() < bigger_sample_sysvar.wens());

        // excess wens shouldn't be reduced by adjust_sysvar_balance_for_rent()
        let excess_wens = smaller_sample_sysvar.wens() + 999;
        smaller_sample_sysvar.set_wens(excess_wens);
        bank.adjust_sysvar_balance_for_rent(&mut smaller_sample_sysvar);
        assert_eq!(smaller_sample_sysvar.wens(), excess_wens);
    }
    ...
    fn test_rent_for_sysvars_adjustment_minimum_genesis_set() {
...

        {
            ...
            assert!(sysvars
                .iter()
                .map(|(_pubkey, account)| dbg!(account.wens()))
                .all(|wens| wens > 1));
        }
    }
    ...
    fn test_rent_for_sysvars_adjustment_full_set() {
        ...
        fn mock_process_instruction(
            ...
            _data: &[u8],
            _invoke_context: &mut InvokeContext,
        ) -> std::result::Result<(), sdk::instruction::InstructionError> {
            Ok(())
        }
            ...
            ) + std::marker::Send,
    {
        ...
        let all_pubkeys: Vec<Pubkey> = std::iter::repeat_with(sdk::pubkey::new_rand)
            .take(total_pubkeys)
            .collect();
        let program_id = system_program::id();
        let starting_wens = 1;
        let starting_account = AccountSharedData::new(starting_wens, 0, &program_id);
                ...
                *accounts_db_caching_enabled,
                move |bank0,
                    ...
                    loop {
                        ...
                        let num_new_banks = 2;
                        let wens = current_minor_fork_bank.slot() + starting_wens + 1;
                            ...
                            .chunks(pubkeys_to_modify.len() / num_new_banks)
                        {
                            ...
                            let account = AccountSharedData::new(wens, 0, &program_id);
                        ...
                        let wens = current_major_fork_bank.slot() + starting_wens + 1;
                        let account = AccountSharedData::new(wens, 0, &program_id);
                ...
                *accounts_db_caching_enabled,
                |bank0,
                 ...
                 starting_wens| {
                    ...
                    loop {
                        let wens_this_round = current_bank.slot() + starting_wens + 1;
                        let account = AccountSharedData::new(wens_this_round, 0, &program_id);
                        for key in pubkeys_to_modify.iter() {
    ...
    fn setup_banks_on_fork_to_remove(
        ...
        for _ in (0..num_banks_on_fork).step_by(step_size) {
            ...
            for i in 0..step_size {
                ...
                if wens_this_round == 0 {
                    wens_this_round = bank_at_fork_tip.bank_id() + starting_wens + 1;
                }
    ...
    fn test_get_inflation_start_slot_devnet_testnet() {
        let GenesisConfigInfo {
            mut genesis_config, ..
        } = create_genesis_config_with_leader(42, &sdk::pubkey::new_rand(), 42);
        genesis_config
            .accounts
        ...
        genesis_config
            .accounts
            .remove(&feature_set::full_inflation::devnet_and_testnet_sino_mainnet::id())
            .unwrap();
        ...
        bank.store_account(
            &feature_set::full_inflation::devnet_and_testnet_sino_mainnet::id(),
            &feature::create_account(
                &Feature {
                    activated_at: Some(2),
                },
    ...
    fn test_stake_vote_account_validity() {
        ...
        let mut vote_account = bank
            ...
            .unwrap_or_default();
        let original_wens = vote_account.wens();
        vote_account.set_wens(0);
        ...
        vote_account.set_wens(original_wens);
    ...
    fn test_get_largest_accounts() {
        ...
        // Initialize accounts; all have larger SOR balances than current Bank built-ins
    ...
    fn test_transfer_sysvar() {
        ...
        fn mock_ix_processor(
            ...
            use sdk::account::WritableAccount;
            let keyed_accounts = invoke_context.get_keyed_accounts()?;
        ...
        #[allow(deprecated)]
        let orig_wens = bank.get_account(&sysvar::clock::id()).unwrap().wens();
        ...
        assert_eq!(
            bank.get_account(&sysvar::clock::id()).unwrap().wens(),
            orig_wens
        );
        info!("{:?}", bank.get_account(&sysvar::clock::id()));
    ...
    fn do_test_clean_dropped_unrooted_banks(freeze_bank1: FreezeBank1) {
        ...
        //! 3. A key with zero wens is _only_ in an unrooted bank (key4)
        //!     - In this case, key4 should be cleaned up
        //! 4. A key with zero wens is in both an unrooted _and_ rooted bank (key5)
        ...
        let key4 = Keypair::new(); // in only bank1, and has zero wens
        let key5 = Keypair::new(); // in both bank1 and bank2, and has zero wens
    ...
    fn test_call_precomiled_program() {
        ...
        let instruction = sdk::secp256k1_instruction::new_secp256k1_instruction(
            &secp_privkey,
            message_arr,
        );
        let tx = Transaction::new_signed_with_payer(
        ...
        let message_arr = b"hello";
        let instruction =
            sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
        let tx = Transaction::new_signed_with_payer(
            &[instruction],
            Some(&mint_keypair.pubkey()),
            &[&mint_keypair],
    ...
    fn test_calculate_fee_compute_units() {
        ...
        let wens_per_signature = fee_structure.wens_per_signature;
        ...
        assert_eq!(
            Bank::calculate_fee(&message, 1, &fee_structure, true),
            max_fee + 3 * wens_per_signature
        );

        // Explicit fee schedule

        let expected_fee_structure = &[
            // (units requested, fee in SOR),
            (0, 0.0),
            (5_000, 0.0),
            (10_000, 0.0),
            (100_000, 0.0),
        ...
        for pair in expected_fee_structure.iter() {
            ...
            assert_eq!(
                fee,
                sor_to_wens(pair.1) + wens_per_signature + ADDITIONAL_FEE
            );
        }
    }
    ...
    fn test_an_empty_instruction_without_program() {
        let (genesis_config, mint_keypair) = create_genesis_config(1);
        let destination = sdk::pubkey::new_rand();
    ...
    fn test_max_accounts_data_size_exceeded() {
        use {
            program_runtime::accounts_data_meter::MAX_ACCOUNTS_DATA_LEN,
            sdk::system_instruction::MAX_PERMITTED_DATA_LENGTH,
        };
        ...
        assert!(matches!(
            ...
            Err(TransactionError::InstructionError(
                _,
                sdk::instruction::InstructionError::MaxAccountsDataSizeExceeded,
            ))
        ));
    }
            ...
            match instruction {
                MockTransferInstruction::Transfer(amount) => {
                    keyed_account_at_index(keyed_accounts, first_instruction_account + 1)?
                        .account
                        .borrow_mut()
                        .checked_sub_wens(amount)?;
                    keyed_account_at_index(keyed_accounts, first_instruction_account + 2)?
                        .account
                        .borrow_mut()
                        .checked_add_wens(amount)?;
                    Ok(())
                }
            }
        } else {
    ...
    fn test_invalid_rent_state_changes_existing_accounts() {
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config_with_leader(sor_to_wens(100.), &Pubkey::new_unique(), 42);
        ...
        let check_account_is_rent_exempt = |pubkey: &Pubkey| -> bool {
            ...
            Rent::default().is_exempt(account.wens(), account.data().len())
        };
    ...
    fn test_drained_created_account() {
        ...
        let transfer_from_instruction = Instruction::new_with_bincode(
            mock_program_id,
            &MockTransferInstruction::Transfer(wens_to_transfer),
            account_metas,
        );
        let tx = Transaction::new_signed_with_payer(
    ...
    fn test_rent_state_changes_sysvars() {
        ...
        let validator_pubkey = sdk::pubkey::new_rand();
        let validator_stake_wens = sor_to_wens(1.);
    ...
    fn test_invalid_rent_state_changes_fee_payer() {
        ...
        genesis_config.fee_rate_governor = FeeRateGovernor::new(
            sdk::fee_calculator::DEFAULT_TARGET_WENS_PER_SIGNATURE,
            sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
        );
    ...
    fn test_rent_state_incinerator() {
        ...
        for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
            bank.transfer(amount, &mint_keypair, &sdk::incinerator::id())
                .unwrap();
        }
    }
    ...
    fn test_resize_and_rent() {
        ...
        let rent_paying_pubkey = sdk::pubkey::new_rand();
        ...
        assert_eq!(
            rent_exempt_minimum_small - 1,
            bank.get_account(&rent_paying_pubkey).unwrap().wens()
        );
