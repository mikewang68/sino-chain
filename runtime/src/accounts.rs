use {
    ...
    log::*,
    rand::{thread_rng, Rng},
    address_lookup_table_program::state::AddressLookupTable,
    sdk::{
...
impl Accounts {
    ...
    pub fn load_lookup_table_addresses(
        ...
        if table_account.owner() == &address_lookup_table_program::id() {
        ...
        if tx.signatures().is_empty() && fee != 0 {
            ...
            for (i, key) in message.account_keys_iter().enumerate() {
                let account = if !message.is_non_loader_key(i) {
                    ...
                    if sdk::sysvar::instructions::check_id(key) {
                        ...
                        tx_rent += rent;
                        rent_debits.insert(key, rent, account.wens());
            ...
            if let Some(payer_index) = payer_index {
                ...
                if payer_account.wens() < fee + min_balance {
            ...
            |collector: &mut BinaryHeap<Reverse<(u64, Pubkey)>>, option| {
                if let Some((pubkey, account, _slot)) = option {
                    ...
                    if collector.len() == num {
                        ...
                        if *entry >= (account.wens(), *pubkey) {
                            return;
                        }
                        collector.pop();
                    }
                    collector.push(Reverse((account.wens(), *pubkey)));
                }
            },
            &ScanConfig::default(),
        )?;
        ...
        self.accounts_db.scan_accounts(
            ...
            |collector: &mut Vec<(Pubkey, AccountSharedData, Slot)>, some_account_tuple| {
                if let Some((pubkey, account, slot)) = some_account_tuple
                    .filter(|(_, account, _)| Self::is_loadable(account.wens()))
                {
...
mod tests {
    use {
        ...
        address_lookup_table_program::state::LookupTableMeta,
        program_runtime::invoke_context::Executors,
        sdk::{
    ...
    fn test_load_accounts_invalid_account_for_fee() {
        ...
        let account = AccountSharedData::new(1, 1, &sdk::pubkey::new_rand()); // <-- owner is not the system program
        accounts.push((key0, account));
    ...
    fn test_load_accounts_fee_payer_is_nonce() {
        ...
        assert_eq!(loaded_transaction.accounts[0].1.wens(), min_balance);

        // Fee leaves zero balance fails
        accounts[0].1.set_wens(min_balance);
        let loaded_accounts = load_accounts_with_fee_and_rent(
            tx.clone(),
            &accounts,
            min_balance,
        ...
        accounts[0].1.set_wens(3 * min_balance / 2);
        let loaded_accounts = load_accounts_with_fee_and_rent(
            tx,
            &accounts,
            min_balance,
    ...
    fn test_load_lookup_table_addresses_invalid_account_data() {
        ...
        let invalid_table_account =
            AccountSharedData::new(1, 0, &address_lookup_table_program::id());
        accounts.store_slow_uncached(0, &invalid_table_key, &invalid_table_account);
    ...
    fn test_instructions() {
        ...
        let instructions_key = sdk::sysvar::instructions::id();
        let keypair = Keypair::new();
        let instructions = vec![CompiledInstruction::new(1, &(), vec![0, 1])];
        let tx = Transaction::new_with_compiled_instructions(
            &[&keypair],
            &[sdk::pubkey::new_rand(), instructions_key],
            Hash::default(),
            vec![native_loader::id()],
            instructions,
        );
    ...
    fn test_overrides() {
        ...
        assert_eq!(loaded_transaction.accounts[1].1.wens(), 42);
    }
    ...
    fn test_nonced_failure_accounts_rollback_from_pays() {
        ...
        let collected_nonce_account = collected_accounts
            ...
            .unwrap();
        assert_eq!(
            collected_nonce_account.wens(),
            nonce_account_pre.wens(),
        );
        assert_matches!(
