...
use {
    crate::{
        ...
        accounts_index::{
            ...
            ScanResult, SlotList, SlotSlice, ZeroWen, ACCOUNTS_INDEX_CONFIG_FOR_BENCHMARKS,
...
pub struct AccountInfo {
    ...
    /// wens in the account used when squashing kept for optimization
...
impl ZeroWen for AccountInfo {
    fn is_zero_wen(&self) -> bool {
        self.wens == 0
    }
}

impl ZeroWen for AccountSharedData {
    fn is_zero_wen(&self) -> bool {
        self.wens() == 0
    }
}
...
impl<'a> LoadedAccount<'a> {
    ...
    pub fn wens(&self) -> u64 {
        match self {
            LoadedAccount::Stored(stored_account_meta) => stored_account_meta.account_meta.wens,
...
pub struct BankHashStats {
    ...
    pub num_wens_stored: u64,
    pub total_data_len: u64,
    pub num_executable_accounts: u64,
}

impl BankHashStats {
    pub fn update<T: ReadableAccount + ZeroWen>(&mut self, account: &T) {
        if account.is_zero_wen() {
        ...
        self.num_wens_stored = self.num_wens_stored.wrapping_add(account.wens());
    }
    ...
    pub fn merge(&mut self, other: &BankHashStats) {
        ...
        self.num_wens_stored = self
            .num_wens_stored
            .wrapping_add(other.num_wens_stored);
...
pub struct AccountsDb {
    ...
    /// such that potentially a 0-wen account update could be present which
    /// means we can remove the account from the index entirely.
    dirty_stores: DashMap<(Slot, AppendVecId), Arc<AccountStorageEntry>>,

    /// Zero-wen accounts that are *not* purged during clean because they need to stay alive
    /// for incremental snapshot support.
    zero_wen_accounts_to_purge_after_full_snapshot: DashSet<(Slot, Pubkey)>,
...
pub fn make_min_priority_thread_pool() -> ThreadPool {
    ...
    rayon::ThreadPoolBuilder::new()
        .thread_name(|i| format!("sino-cleanup-accounts-{}", i))
...
impl frozen_abi::abi_example::AbiExample for AccountsDb {
    fn example() -> Self {
...
impl<'a> ZeroWen for StoredAccountMeta<'a> {
    fn is_zero_wen(&self) -> bool {
        self.wens() == 0
    }
}
...
impl AccountsDb {
    ...
    fn default_with_accounts_index(
        ...
        AccountsDb {
            ...
            thread_pool: rayon::ThreadPoolBuilder::new()
                .num_threads(num_threads)
                .thread_name(|i| format!("sino-db-accounts-{}", i))
                .build()
                .unwrap(),
            ...
            zero_wen_accounts_to_purge_after_full_snapshot: DashSet::default(),
    ...
    fn start_background_hasher(&mut self) {
        ...
        Builder::new()
            .name("sino-db-store-hasher-accounts".to_string())
            .spawn(move || {
    ...
    fn construct_candidate_clean_keys(
        ...
        // Check if we should purge any of the zero_wen_accounts_to_purge_later, based on the
        // last_full_snapshot_slot.
        assert!(
            last_full_snapshot_slot.is_some() || self.zero_wen_accounts_to_purge_after_full_snapshot.is_empty(),
            "if snapshots are disabled, then zero_wen_accounts_to_purge_later should always be empty"
        );
        if let Some(last_full_snapshot_slot) = last_full_snapshot_slot {
            self.zero_wen_accounts_to_purge_after_full_snapshot
                .retain(|(slot, pubkey)| {
    ...
    // Purge zero wen accounts and older rooted account states as garbage
    // collection
    ...
    pub fn clean_accounts(
        ...
        let (mut purges_zero_wens, purges_old_accounts) = {
            let do_clean_scan = || {
                pubkeys
                    .par_chunks(4096)
                    .map(|pubkeys: &[Pubkey]| {
                        let mut purges_zero_wens = HashMap::new();
                                ...
                                if !exists {
                                    ...
                                    match index_in_slot_list {
                                        Some(index_in_slot_list) => {
                                            ...
                                            if account_info.wens == 0 {
                                                useless = false;
                                                purges_zero_wens.insert(
                                                    *pubkey,
                                                    (
                        ...
                        (purges_zero_wens, purges_old_accounts)
                    })
                    .reduce(
                        || (HashMap::new(), Vec::new()),
                        |mut m1, m2| {
        ...
        for (key, (account_infos, ref_count)) in purges_zero_wens.iter_mut() {
        ...
        Self::calc_delete_dependencies(&purges_zero_wens, &mut store_counts);
        calc_deps_time.stop();

        let mut purge_filter = Measure::start("purge_filter");
        self.filter_zero_wen_clean_for_incremental_snapshots(
            max_clean_root,
            last_full_snapshot_slot,
            &store_counts,
            &mut purges_zero_wens,
        );
        purge_filter.stop();

        let mut reclaims_time = Measure::start("reclaims");
        // Recalculate reclaims with new purge set
        let pubkey_to_slot_set: Vec<_> = purges_zero_wens
            .into_iter()
    ...
    /// During clean, some zero-wen accounts that are marked for purge should *not* actually
    /// get purged.  Filter out those accounts here.
    ///
    /// When using incremental snapshots, do not purge zero-wen accounts if the slot is higher
    ...
    fn filter_zero_wen_clean_for_incremental_snapshots(
        &self,
        max_clean_root: Option<Slot>,
        last_full_snapshot_slot: Option<Slot>,
        store_counts: &HashMap<AppendVecId, (usize, HashSet<Pubkey>)>,
        purges_zero_wens: &mut HashMap<Pubkey, (SlotList<AccountInfo>, RefCount)>,
    ) {
        ...
        purges_zero_wens.retain(|pubkey, (slot_account_infos, _ref_count)| {
            // Only keep purges_zero_wens where the entire history of the account in the root set
            ...
            slot_account_info_at_highest_slot.map_or(true, |(slot, account_info)| {
                // Do *not* purge zero-wen accounts if the slot is greater than the last full
                ...
                assert!(account_info.is_zero_wen());
        ...
        A: Default,
    {
        let key = match &index_key {
            ...
            IndexKey::SinoAccountStorage(key) => key,
            IndexKey::SinoAccountOwner(key) => key,
            IndexKey::SinoAccountOperational(key) => key,
            IndexKey::SinoRelyingOwner(key) => key,
        };
                ...
                LoadedAccountAccessor::Stored(None) => {
                    match load_hint {
                        LoadHint::FixedMaxRoot => {
                            ...
                            // 2) The `pubkey` asked for in this function is a zero-wen account,
                            // and the storage entry holding this account qualified for zero-wen clean.
                            //
    ...
    fn hash_account_data(
        ...
        hasher.update(&wens.to_le_bytes());
    ...
    >(
        &self,
        slot: Slot,
        accounts: &[(&Pubkey, &(impl ReadableAccount + ZeroWen))],
        hashes: Option<&[impl Borrow<Hash>]>,
        storage_finder: F,
        ...
        let accounts_and_meta_to_store: Vec<_> = accounts
            ...
            .map(|(pubkey, account)| {
                ...
                // None = store a default/empty account with 0 wens
                let (account, data_len) = if account.is_zero_wen() {
                    (None, 0)
                } else {
    ...
    fn calculate_accounts_hash(
        ...
        // We'll also accumulate the wens within each chunk and fewer chunks results in less contention to accumulate the sum.
        let chunks = crate::accounts_hash::MERKLE_FANOUT.pow(4);
        let total_wens = Mutex::<u64>::new(0);
        let get_hashes = || {
            keys.par_chunks(chunks)
                .map(|pubkeys| {
                    ...
                    let result: Vec<Hash> = pubkeys
                        ...
                        .collect();
                    let mut total = total_wens.lock().unwrap();
                    *total =
        ...
        scan.stop();
        let total_wens = *total_wens.lock().unwrap();
        ...
        Ok((accumulated_hash, total_wens))
    }
    ...
    fn calculate_accounts_hash_helper_with_verify(
        ...
        let (hash, total_wens) = self.calculate_accounts_hash_helper(
            use_index,
            slot,
            ancestors,
            check_hash,
        ...
        if debug_verify {
            ...
            let (hash_other, total_wens_other) = self.calculate_accounts_hash_helper(
                !use_index,
                slot,
                ancestors,
                check_hash,
            ...
            let success = hash == hash_other
                && total_wens == total_wens_other
                && total_wens == expected_capitalization.unwrap_or(total_wens);
            assert!(success, "update_accounts_hash_with_index_option mismatch. hashes: {}, {}; wens: {}, {}; expected wens: {:?}, using index: {}, slot: {}", hash, hash_other, total_wens, total_wens_other, expected_capitalization, use_index, slot);
        }
        Ok((hash, total_wens))
    }
    ...
    fn scan_snapshot_stores_with_cache(
        ...
        let result: Vec<BinnedHashData> = Self::scan_account_storage_no_bank(
            ...
            |loaded_account: LoadedAccount, accum: &mut BinnedHashData, slot: Slot| {
                ...
                let raw_wens = loaded_account.wens();
                let zero_raw_wens = raw_wens == 0;
                let balance = if zero_raw_wens {
                    crate::accounts_hash::ZERO_RAW_WENS_SENTINEL
                } else {
                    raw_wens
                };
    ...
    pub fn calculate_accounts_hash_without_index(
        ...
        let mut scan_and_hash = move || {
            ...
            for pass in 0..num_hash_scan_passes {
                ...
                let (hash, wens, for_next_pass) = hash.rest_of_hash_calculation(
                    result,
                    &mut stats,
    ...
    pub fn verify_bank_hash_and_wens(
        ...
        let (calculated_hash, calculated_wens) = self
            .calculate_accounts_hash_helper_with_verify(
                use_index,
        ...
        if calculated_wens != total_wens {
            warn!(
                "Mismatched total wens: {} calculated: {}",
                total_wens, calculated_wens
            );
            return Err(MismatchedTotalWens(calculated_wens, total_wens));
        }
    ...
    fn store_accounts_frozen<'a, T: ReadableAccount + Sync + ZeroWen>(
        &'a self,
        slot: Slot,
        accounts: &[(&Pubkey, &T)],
        hashes: Option<&[impl Borrow<Hash>]>,
    ...
    fn store_accounts_custom<'a, T: ReadableAccount + Sync + ZeroWen>(
        &'a self,
        slot: Slot,
        accounts: &[(&Pubkey, &T)],
        hashes: Option<&[impl Borrow<Hash>]>,
            ...
            |(
                ...
                (
                    ...
                    AccountInfo {
                        ...
                        wens: stored_account.account_meta.wens,
                    },
                )
            },
        );
...
pub mod tests {
    ...
    fn test_accountsdb_calculate_accounts_hash_without_index() {
        ...
        let sum = raw_expected.iter().map(|item| item.wens).sum();
        let result = AccountsDb::calculate_accounts_hash_without_index(
            TempDir::new().unwrap().path(),
            &get_storage_refs(&storages),
            None,
    ...
    fn create_account(
        ...
        for t in 0..num_vote {
            let pubkey = sdk::pubkey::new_rand();
            let account =
                AccountSharedData::new((num + t + 1) as u64, space, &vote_program::id());
            pubkeys.push(pubkey);
            let ancestors = vec![(slot, 0)].into_iter().collect();
            assert!(accounts
                .load_without_fixed_root(&ancestors, &pubkey)
    ...
    fn update_accounts(accounts: &AccountsDb, pubkeys: &[Pubkey], slot: Slot, range: usize) {
        for _ in 1..1000 {
            ...
            {
                account.checked_add_wens(1).unwrap();
    ...
    #[test]
    fn test_clean_zero_wen_and_dead_slot() {
        sino_logger::setup();
        ...
        // Update account 1 as  zero wens account
        accounts.store_uncached(2, &[(&pubkey1, &zero_wen_account)]);
        ...
        // zero wens, and are not present in any other slot's
        // storage entries
        assert_eq!(accounts.alive_account_count_in_slot(1), 0);
    }

    #[test]
    fn test_clean_multiple_zero_wen_decrements_index_ref_count() {
        sino_logger::setup();
        ...
        accounts.store_uncached(0, &[(&pubkey1, &zero_wen_account)]);
        accounts.store_uncached(0, &[(&pubkey2, &zero_wen_account)]);
        accounts.store_uncached(1, &[(&pubkey1, &zero_wen_account)]);
        accounts.store_uncached(2, &[(&pubkey1, &zero_wen_account)]);
        // Root all slots
        accounts.add_root(0);
        accounts.add_root(1);
        ...
        // accounts are zero wens
        assert!(accounts.storage.get_slot_stores(0).is_none());
        assert!(accounts.storage.get_slot_stores(1).is_none());
        // Slot 2 only has a zero wen account as well. But, calc_delete_dependencies()
    ...
    #[test]
    fn test_clean_zero_wen_and_old_roots() {
        sino_logger::setup();
        ...
        // Store a zero-wen account
        accounts.store_uncached(0, &[(&pubkey, &account)]);
        accounts.store_uncached(1, &[(&pubkey, &zero_wen_account)]);

        // Simulate rooting the zero-wen account, should be a
        // candidate for cleaning
        accounts.add_root(0);
        accounts.add_root(1);

        // Slot 0 should be removed, and
        // zero-wen account should be cleaned
        accounts.clean_accounts(None, false, None);
        ...
        // zero wen account, should no longer exist in accounts index
    ...
    #[test]
    fn test_clean_old_with_zero_wen_account() {
        sino_logger::setup();
        ...
        //Old state behind zero-wen account is cleaned up
        assert_eq!(accounts.alive_account_count_in_slot(0), 0);
        assert_eq!(accounts.alive_account_count_in_slot(1), 2);
    }
    ...
    fn test_clean_old_with_both_normal_and_zero_wen_accounts() {
        sino_logger::setup();
        ...
        //both zero wen and normal accounts are cleaned up
        assert_eq!(accounts.alive_account_count_in_slot(0), 0);
        // The only store to slot 1 was a zero wen account, should
        // be purged by zero-wen cleaning logic because slot 1 is
        // rooted
        assert_eq!(accounts.alive_account_count_in_slot(1), 0);
        assert_eq!(accounts.alive_account_count_in_slot(2), 1);

        // `pubkey1`, a zero wen account, should no longer exist in accounts index
    ...
    #[test]
    fn test_clean_max_slot_zero_wen_account() {
        sino_logger::setup();
        ...
        // store an account, make it a zero wen account
        // in slot 1
        accounts.store_uncached(0, &[(&pubkey, &account)]);
        ...
        // The zero wen account, should no longer exist in accounts index
        // because it has been removed
        assert!(accounts.accounts_index.get(&pubkey, None, None).is_none());
    }
    ...
    fn test_accounts_db_serialize1() {
        ...
        // Overwrite account 30 from slot 0 with wens=0 into slot 1.
        ...
        accounts.clean_accounts(None, false, None);
        // Overwrite account 31 from slot 0 with wens=0 into slot 2.
        ...
        // accounts 30 and  31 (overwritten with zero-wen accounts in slot 1 and
    ...
    fn assert_load_account(
        ...
        let (account, slot) = accounts
            .load_without_fixed_root(&ancestors, &pubkey)
            .unwrap();
        assert_eq!((account.wens(), slot), (expected_wens, slot));
    }
    ...
    fn test_accounts_db_purge_keep_live() {
        ...
        let account = AccountSharedData::new(some_wen, no_data, &owner);
        let pubkey = sdk::pubkey::new_rand();

        let account2 = AccountSharedData::new(some_wen, no_data, &owner);
        let pubkey2 = sdk::pubkey::new_rand();

        let zero_wen_account = AccountSharedData::new(zero_wen, no_data, &owner);
        ...
        // Step B
        current_slot += 1;
        let zero_wen_slot = current_slot;
        accounts.store_uncached(current_slot, &[(&pubkey, &zero_wen_account)]);
        accounts.add_root(current_slot);

        assert_load_account(&accounts, current_slot, pubkey, zero_wen);
        ...
        // Zero wen entry was not the one purged
        assert_eq!(index_slot, zero_wen_slot);
    ...
    fn test_accounts_db_serialize_zero_and_free() {
        ...
        let account2 = AccountSharedData::new(some_wen + 1, no_data, &owner);
        let pubkey2 = sdk::pubkey::new_rand();

        let filler_account = AccountSharedData::new(some_wen, no_data, &owner);
        let filler_account_pubkey = sdk::pubkey::new_rand();
    ...
    fn with_chained_zero_wen_accounts<F>(f: F)
    where
        F: Fn(AccountsDb, Slot) -> AccountsDb,
    {
        ...
        let account = AccountSharedData::new(some_wen, no_data, &owner);
        let account2 = AccountSharedData::new(some_wen + 100_001, no_data, &owner);
        let account3 = AccountSharedData::new(some_wen + 100_002, no_data, &owner);
        let zero_wen_account = AccountSharedData::new(zero_wen, no_data, &owner);

        let pubkey = sdk::pubkey::new_rand();
        let purged_pubkey1 = sdk::pubkey::new_rand();
        let purged_pubkey2 = sdk::pubkey::new_rand();

        let dummy_account = AccountSharedData::new(dummy_wen, no_data, &owner);
        ...
        current_slot += 1;
        accounts.store_uncached(current_slot, &[(&purged_pubkey1, &zero_wen_account)]);
        accounts.store_uncached(current_slot, &[(&purged_pubkey2, &account3)]);
        accounts.add_root(current_slot);

        current_slot += 1;
        accounts.store_uncached(current_slot, &[(&purged_pubkey2, &zero_wen_account)]);
        accounts.add_root(current_slot);
        ...
        assert_load_account(&accounts, current_slot, pubkey, some_wen);
        assert_load_account(&accounts, current_slot, purged_pubkey1, 0);
        assert_load_account(&accounts, current_slot, purged_pubkey2, 0);
        assert_load_account(&accounts, current_slot, dummy_pubkey, dummy_wen);

        accounts
            .verify_bank_hash_and_wens(4, &Ancestors::default(), 1222, true)
            .unwrap();
    }
    ...
    fn test_accounts_purge_chained_purge_before_snapshot_restore() {
        sino_logger::setup();
        with_chained_zero_wen_accounts(|accounts, current_slot| {
            accounts.clean_accounts(None, false, None);
                ...
                std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        loop {
                            ...
                            assert_eq!(slot, slot);
                            assert_eq!(account.wens(), account_bal);
                            i += 1;
                        }
                    })
                    .unwrap()
    ...
    fn test_bank_hash_stats() {
        ...
        account.checked_sub_wens(1).unwrap();
        ...
        assert_eq!(bank_hash.stats.num_wens_stored, 1);
    ...
    fn test_verify_bank_hash() {
        ...
        assert_matches!(
            db.verify_bank_hash_and_wens(some_slot, &ancestors, 1, true),
            Ok(_)
        );
    ...
    fn test_verify_bank_capitalization() {
        ...
        let native_account_pubkey = sdk::pubkey::new_rand();
        db.store_uncached(
            some_slot,
            &[(
                &native_account_pubkey,
                &sdk::native_loader::create_loadable_account_for_test("foo"),
            )],
        );
        db.update_accounts_hash_test(some_slot, &ancestors);
        assert_matches!(
            db.verify_bank_hash_and_wens(some_slot, &ancestors, 2, true),
            Ok(_)
        );

        assert_matches!(
            db.verify_bank_hash_and_wens(some_slot, &ancestors, 10, true),
            Err(MismatchedTotalWens(expected, actual)) if expected == 2 && actual == 10
        );
    }
    ...
    fn test_verify_bank_hash_no_account() {
        ...
        db.update_accounts_hash_test(some_slot, &ancestors);
        assert_matches!(
            db.verify_bank_hash_and_wens(some_slot, &ancestors, 0, true),
            Ok(_)
        );
    }
    ...
    fn test_storage_finder() {
        ...
        let wens = 100;
        let data_len = 8190;
        let account = AccountSharedData::new(wens, data_len, &sdk::pubkey::new_rand());
    ...
    fn test_accounts_purge_long_chained_after_snapshot_restore() {
        ...
        let account = AccountSharedData::new(old_wen, no_data, &owner);
        let account2 = AccountSharedData::new(old_wen + 100_001, no_data, &owner);
        let account3 = AccountSharedData::new(old_wen + 100_002, no_data, &owner);
        let dummy_account = AccountSharedData::new(99_999_999, no_data, &owner);
        let zero_wen_account = AccountSharedData::new(zero_wen, no_data, &owner);

        let pubkey = sdk::pubkey::new_rand();
        let dummy_pubkey = sdk::pubkey::new_rand();
        let purged_pubkey1 = sdk::pubkey::new_rand();
        let purged_pubkey2 = sdk::pubkey::new_rand();
        ...
        assert_load_account(&accounts, current_slot, pubkey, old_wen);
        assert_load_account(&accounts, current_slot, purged_pubkey1, 0);
        assert_load_account(&accounts, current_slot, purged_pubkey2, 0);
    }
    ...
    fn do_full_clean_refcount(store1_first: bool, store_size: u64) {
        ...
        let account = AccountSharedData::new(old_wen, data_size, &owner);
        let account2 = AccountSharedData::new(old_wen + 100_001, data_size, &owner);
        let account3 = AccountSharedData::new(old_wen + 100_002, data_size, &owner);
        let account4 = AccountSharedData::new(dummy_wen, data_size, &owner);
        let zero_wen_account = AccountSharedData::new(zero_wen, data_size, &owner);
        ...
        // D: Make all keys 0-wen, cleans all keys
        current_slot += 1;
        assert_eq!(3, accounts.ref_count_for_pubkey(&pubkey1));
        accounts.store_uncached(current_slot, &[(&pubkey1, &zero_wen_account)]);
        accounts.store_uncached(current_slot, &[(&pubkey2, &zero_wen_account)]);
        accounts.store_uncached(current_slot, &[(&pubkey3, &zero_wen_account)]);
    ...
    fn test_accounts_clean_after_snapshot_restore_then_old_revives() {
        ...
        // D: Make pubkey1 0-wen; also triggers clean of step B
        current_slot += 1;
        assert_eq!(3, accounts.ref_count_for_pubkey(&pubkey1));
        accounts.store_uncached(current_slot, &[(&pubkey1, &zero_wen_account)]);
        accounts.clean_accounts(None, false, None);
        ...
        assert_load_account(&accounts, current_slot, pubkey1, zero_wen);
        assert_load_account(&accounts, current_slot, pubkey2, old_wen);
        assert_load_account(&accounts, current_slot, dummy_pubkey, dummy_wen);
    ...
    fn test_shrink_stale_slots_processed() {
        ...
        for startup in &[false, true] {
            ...
            accounts
                .verify_bank_hash_and_wens(current_slot, &no_ancestors, 22300, true)
                .unwrap();
    ...
    fn test_account_balance_for_capitalization_sysvar() {
        let normal_sysvar = sdk::account::create_account_for_test(
            &sdk::slot_history::SlotHistory::default(),
        );
        assert_eq!(normal_sysvar.wens(), 1);
    }
    ...
    fn test_account_balance_for_capitalization_native_program() {
        let normal_native_program =
            sdk::native_loader::create_loadable_account_for_test("foo");
        assert_eq!(normal_native_program.wens(), 1);
    }
    ...
    #[test]
    fn test_zero_wen_new_root_not_cleaned() {
        let db = AccountsDb::new(Vec::new(), &ClusterType::Development);
        let account_key = Pubkey::new_unique();
        let zero_wen_account =
            AccountSharedData::new(0, 0, AccountSharedData::default().owner());

        // Store zero wen account into slots 0 and 1, root both slots
        db.store_uncached(0, &[(&account_key, &zero_wen_account)]);
        db.store_uncached(1, &[(&account_key, &zero_wen_account)]);
        db.get_accounts_delta_hash(0);
        db.add_root(0);
        db.get_accounts_delta_hash(1);
        db.add_root(1);

        // Only clean zero wen accounts up to slot 0
        db.clean_accounts(Some(0), false, None);

        // Should still be able to find zero wen account in slot 1
        assert_eq!(
            db.load_without_fixed_root(&Ancestors::default(), &account_key),
            Some((zero_wen_account, 1))
        );
    }
    ...
    fn test_flush_accounts_cache() {
        ...
        let root5 = 5;
        let root6 = 6;
        let unrooted_key = sdk::pubkey::new_rand();
        let key5 = sdk::pubkey::new_rand();
        let key6 = sdk::pubkey::new_rand();
    ...
    fn test_read_only_accounts_cache() {
        ...
        db.store_cached(0, &[(&account_key, &zero_wen_account)]);
        ...
        assert_eq!(account.wens(), 1);
        assert_eq!(db.read_only_accounts_cache.cache_len(), 1);
        db.store_cached(2, &[(&account_key, &zero_wen_account)]);
        assert_eq!(db.read_only_accounts_cache.cache_len(), 1);
        let account = db
            .load_with_fixed_root(&Ancestors::default(), &account_key)
            .map(|(account, _)| account)
    ...
    #[test]
    fn test_flush_cache_dont_clean_zero_wen_account() {
        let caching_enabled = true;
        let db = Arc::new(AccountsDb::new_with_config_for_tests(
            Vec::new(),
            &ClusterType::Development,
        ...
        let zero_wen_account_key = Pubkey::new_unique();
        let other_account_key = Pubkey::new_unique();

        let original_wens = 1;
        let slot0_account =
            AccountSharedData::new(original_wens, 1, AccountSharedData::default().owner());
        let zero_wen_account =
            AccountSharedData::new(0, 0, AccountSharedData::default().owner());

        // Store into slot 0, and then flush the slot to storage
        db.store_cached(0, &[(&zero_wen_account_key, &slot0_account)]);
        // Second key keeps other wen account entry for slot 0 alive,
        // preventing clean of the zero_wen_account in slot 1.
        ...
        db.store_cached(1, &[(&zero_wen_account_key, &zero_wen_account)]);
        ...
        // the refcount of `zero_wen_account_key` because cached keys do not bump
        // the refcount in the index. This means clean should *not* remove
        // `zero_wen_account_key` from slot 2
        db.store_cached(2, &[(&zero_wen_account_key, &zero_wen_account)]);
        db.add_root(1);
        db.add_root(2);
        ...
        // The `zero_wen_account_key` is still alive in slot 1, so refcount for the
        // pubkey should be 2
        assert_eq!(
            db.accounts_index
                .ref_count_from_storage(&zero_wen_account_key),
            2
        );
        assert_eq!(
        ...
        // The zero-wen account in slot 2 should not be purged yet, because the
        // entry in slot 1 is blocking cleanup of the zero-wen account.
        let max_root = None;
    ...
    fn test_scan_flush_accounts_cache_then_clean_drop() {
        ...
        /*
            Store zero wen account into slots 0, 1, 2 where
        ...
        */
        ...
        let account = db
            ...
            .unwrap();
        assert_eq!(account.0.wens(), zero_wen_account.wens());
        ...
        let account = db
            ...
            .unwrap();
        assert_eq!(account.0.wens(), slot1_account.wens());
    ...
    fn start_load_thread(
        ...
        exit: Arc<AtomicBool>,
        pubkey: Arc<Pubkey>,
        expected_wens: impl Fn(&(AccountSharedData, Slot)) -> u64 + Send + 'static,
    ) -> JoinHandle<()> {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                loop {
                    ...
                    // slot + 1 == account.wens because of the account-cache-flush thread
                    assert_eq!(
                        loaded_account.0.wens(),
                        expected_wens(&loaded_account)
                    );
                }
            })
            .unwrap()
    ...
    fn test_calculate_storage_count_and_alive_bytes() {
        let accounts = AccountsDb::new_single_for_tests();
        let shared_key = sdk::pubkey::new_rand();
        let account = AccountSharedData::new(1, 1, AccountSharedData::default().owner());
        let slot0 = 0;
    ...
    fn test_calculate_storage_count_and_alive_bytes_2_accounts() {
        let accounts = AccountsDb::new_single_for_tests();
        let keys = [
            sdk::pubkey::Pubkey::new(&[0; 32]),
            sdk::pubkey::Pubkey::new(&[255; 32]),
        ];
    ...
    fn test_purge_alive_unrooted_slots_after_clean() {
        ...
        // Store accounts with greater than 0 wens
        let account = AccountSharedData::new(1, 1, AccountSharedData::default().owner());
        accounts.store_uncached(slot0, &[(&shared_key, &account)]);
        ...
        // On the next *rooted* slot, update the `shared_key` account to zero wens
        let zero_wen_account =
            AccountSharedData::new(0, 0, AccountSharedData::default().owner());
        accounts.store_uncached(slot1, &[(&shared_key, &zero_wen_account)]);

        // Simulate adding dirty pubkeys on bank freeze, set root
        accounts.get_accounts_delta_hash(slot1);
        accounts.add_root(slot1);

        // The later rooted zero-wen update to `shared_key` cannot be cleaned
    ...
    #[test]
    fn test_filter_zero_wen_clean_for_incremental_snapshots() {
        sino_logger::setup();
        let slot = 10;
        ...
        let do_test = |test_params: TestParameters| {
            ...
            let mut purges_zero_wens = HashMap::default();
            purges_zero_wens.insert(pubkey, (vec![(slot, account_info)], 1));

            let accounts_db = AccountsDb::new_single_for_tests();
            accounts_db.filter_zero_wen_clean_for_incremental_snapshots(
            ...
            assert_eq!(
                purges_zero_wens.contains_key(&pubkey),
        ...
        // Scenario 2: last full snapshot is GREATER THAN zero wen account slot
        ...
        // Scenario 3: last full snapshot is EQUAL TO zero wen account slot
        ...
        // Scenario 4: last full snapshot is LESS THAN zero wen account slot
