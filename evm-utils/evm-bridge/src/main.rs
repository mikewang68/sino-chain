...
use derivative::*;
use evm_loader_program::instructions::FeePayerType;
...
impl EvmBridge {
    fn new(
        ...
        let key = sdk::signature::read_keypair_file(keypath).unwrap();
    ...
    /// Wrap evm tx into sino, optionally add meta keys, to sino signature.
    async fn send_tx(
        &self,
        tx: evm::Transaction,
    ...
    fn make_send_tx_instructions(
        ...
        let mut ix = if self.borsh_encoding {
            ...
            evm_loader_program::send_raw_tx(
                self.key.pubkey(),
                tx.clone(),
            ...
            evm_loader_program::send_raw_tx_old(
                self.key.pubkey(),
                tx.clone(),
                Some(self.key.pubkey()),
            )
        ...
        if native_fee_used {
            vec![
                system_instruction::assign(&self.key.pubkey(), &sdk::evm_loader::ID),
                ix,
                evm_loader_program::free_ownership(self.key.pubkey()),
            ]
        } else {
            vec![ix]
        }
    ...
    fn make_send_big_tx_instructions(
        ...
        let ix = if self.borsh_encoding {
            ...
            evm_loader_program::big_tx_execute(storage_pubkey, Some(&payer_pubkey), fee_type)
        } else {
            evm_loader_program::big_tx_execute_old(storage_pubkey, Some(&payer_pubkey))
        };
        if native_fee_used {
            vec![
                system_instruction::assign(&self.key.pubkey(), &sdk::evm_loader::ID),
...
impl BridgeERPC for BridgeErpcImpl {
    ...
    fn sign(&self, meta: Self::Metadata, address: Hex<Address>, data: Bytes) -> EvmResult<Bytes> {
        ...
        let hash_to_sign = sdk::keccak::hash(&message_data);
...
impl ChainERPC for ChainErpcProxy {          //ChainERPC内的接口函数有一部分功能未完成，此处未添加
    type Metadata = Arc<EvmBridge>;
...
pub(crate) fn from_client_error(client_error: ClientError) -> evm_rpc::Error {
    ...
    match client_error_kind {
        ClientErrorKind::RpcError(client::rpc_request::RpcError::RpcResponseError {
            code,
            message,
            data,
            original_err,
...
async fn main(args: Args) -> StdResult<(), Box<dyn std::error::Error>> {
    ...
    let keyfile_path = args
        .keyfile
        .unwrap_or_else(|| cli_config::Config::default().keypair_path);
