...
impl Default for MetricsAgent {
    fn default() -> Self {
        let max_points_per_sec = env::var("METRICS_MAX_POINTS_PER_SECOND")
            .map(|x| {
                x.parse()
                    .expect("Failed to parse METRICS_MAX_POINTS_PER_SECOND")
            })
            .unwrap_or(4000);
...
fn get_metrics_config() -> Result<MetricsConfig, String> {
    ...
    let config_var = env::var("METRICS_CONFIG")
        .map_err(|err| format!("METRICS_CONFIG: {}", err))?;

    for pair in config_var.split(',') {
        let nv: Vec<_> = pair.split('=').collect();
        if nv.len() != 2 {
            return Err(format!("METRICS_CONFIG is invalid: '{}'", pair));
        }
        ...
        match nv[0] {
            ...
            "u" => config.username = v,
            "p" => config.password = v,
            _ => return Err(format!("METRICS_CONFIG is invalid: '{}'", pair)),
        }
    }

    if !config.complete() {
        return Err("METRICS_CONFIG is incomplete".to_string());
    }
    Ok(config)
}
...
pub fn set_panic_hook(program: &'static str, version: Option<String>) {
    ...
    SET_HOOK.call_once(|| {
        ...
        std::panic::set_hook(Box::new(move |ono| {
            ...
            if thread_name != "sino-rpc" && thread_name != "sino-evm-state-rpc-worker" {
