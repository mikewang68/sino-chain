...
pub trait SyscallStubs: Sync + Send {
    ...
    fn sor_log_compute_units(&self) {
        sor_log("SyscallStubs: sor_log_compute_units() not available");
    }
    ...
    /// # Safety
    unsafe fn sor_memcpy(&self, dst: *mut u8, src: *const u8, n: usize) {
    ...
    /// # Safety
    unsafe fn sor_memmove(&self, dst: *mut u8, src: *const u8, n: usize) {
        std::ptr::copy(src, dst, n);
    }
    /// # Safety
    unsafe fn sor_memcmp(&self, s1: *const u8, s2: *const u8, n: usize, result: *mut i32) {
        let mut i = 0;
        while i < n {
            let a = *s1.add(i);
            let b = *s2.add(i);
    ...
    /// # Safety
    unsafe fn sor_memset(&self, s: *mut u8, c: u8, n: usize) {
...
pub(crate) fn sor_log(message: &str) {
    SYSCALL_STUBS.read().unwrap().sor_log(message);
}

pub(crate) fn sor_log_64(arg1: u64, arg2: u64, arg3: u64, arg4: u64, arg5: u64) {
    sor_log(&format!(
        "{:#x}, {:#x}, {:#x}, {:#x}, {:#x}",
...
pub(crate) fn sor_log_compute_units() {
    SYSCALL_STUBS.read().unwrap().sor_log_compute_units();
}

pub(crate) fn sor_invoke_signed(
...
pub(crate) fn sor_memcpy(dst: *mut u8, src: *const u8, n: usize) {
    unsafe {
        SYSCALL_STUBS.read().unwrap().sor_memcpy(dst, src, n);
    }
}

pub(crate) fn sor_memmove(dst: *mut u8, src: *const u8, n: usize) {
    unsafe {
        SYSCALL_STUBS.read().unwrap().sor_memmove(dst, src, n);
    }
}

pub(crate) fn sor_memcmp(s1: *const u8, s2: *const u8, n: usize, result: *mut i32) {
    unsafe {
        SYSCALL_STUBS.read().unwrap().sor_memcmp(s1, s2, n, result);
    }
}

pub(crate) fn sor_memset(s: *mut u8, c: u8, n: usize) {
    unsafe {
        SYSCALL_STUBS.read().unwrap().sor_memset(s, c, n);
    }
}
