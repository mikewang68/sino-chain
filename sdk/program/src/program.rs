        ...
        for account_info in account_infos.iter() {
            if account_meta.pubkey == *account_info.key {
                if account_meta.is_writable {
                    let _ = account_info.try_borrow_mut_wens()?;
                    let _ = account_info.try_borrow_mut_data()?;
                } else {
                    let _ = account_info.try_borrow_wens()?;
...
pub fn invoke_signed_unchecked(
    ...
    crate::program_stubs::sor_invoke_signed(instruction, account_infos, signers_seeds)
}

/// Maximum size that can be set using sor_set_return_data()
...
pub fn set_return_data(data: &[u8]) {
    ...
    {
        extern "C" {
            fn sor_set_return_data(data: *const u8, length: u64);
        }

        unsafe { sor_set_return_data(data.as_ptr(), data.len() as u64) };
    }

    #[cfg(not(target_arch = "bpf"))]
    crate::program_stubs::sor_set_return_data(data)
}
...
pub fn get_return_data() -> Option<(Pubkey, Vec<u8>)> {
    #[cfg(target_arch = "bpf")]
    {
        ...
        extern "C" {
            fn sor_get_return_data(data: *mut u8, length: u64, program_id: *mut Pubkey) -> u64;
        }
        ...
        let size =
            unsafe { sor_get_return_data(buf.as_mut_ptr(), buf.len() as u64, &mut program_id) };
    ...
    #[cfg(not(target_arch = "bpf"))]
    crate::program_stubs::sor_get_return_data()
}
