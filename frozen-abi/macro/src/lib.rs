...
#[cfg(RUSTC_WITH_SPECIALIZATION)]
use syn::{
    parse_macro_input, AttributeArgs, Error, Ident, Item, ItemEnum, ItemStruct,
    ItemType, Lit, Meta, NestedMeta, 
};
...
//         #( #attrs )*
//         impl #impl_generics ::frozen_abi::abi_example::AbiExample for #type_name #ty_generics #where_clause {
//             fn example() -> Self {
//                 ::log::info!(
//                     "AbiExample for enum: {}",
...
//     (quote! {
//         impl #impl_generics ::frozen_abi::abi_example::AbiEnumVisitor for #type_name #ty_generics #where_clause {
//             fn visit_for_abi(&self, digester: &mut ::frozen_abi::abi_digester::AbiDigester) -> ::frozen_abi::abi_digester::DigestResult {
...
fn quote_for_test(
    ...
    quote! {
        ...
        mod #test_mod_ident {
            use super::*;
            use ::frozen_abi::abi_example::{AbiExample, AbiEnumVisitor};
            ...
            fn test_abi_digest() {
                ...
                if ::std::env::var("SINO_ABI_BULK_UPDATE").is_ok() {
                    if #expected_digest != actual_digest {
                        #p!("sed -i -e 's/{}/{}/g' $(git grep --files-with-matches frozen_abi)", #expected_digest, hash);
                    }
                    ::log::warn!("Not testing the abi digest under SINO_ABI_BULK_UPDATE!");
                } else {
                    if let Ok(dir) = ::std::env::var("SINO_ABI_DUMP_DIR") {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Examine the diff in SINO_ABI_DUMP_DIR!: $ diff -u {}/*{}* {}/*{}*", dir, #expected_digest, dir, actual_digest);
                    } else {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Confirm the diff by rerunning before and after this test failed with SINO_ABI_DUMP_DIR!");
                    }
                }
            }
        }
